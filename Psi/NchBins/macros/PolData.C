...
TH1F *Reco_StatEv;
TH1F *Reco_Onia_mass[onia::NchBins+1][onia::kNbPTMaxBins+1][onia::kNbRapForPTBins+1];
TH1F *Reco_Onia_CPMPU[onia::NchBins+1][25]; //////second number is number of pile up bins
TH2F *Reco_Onia_rap_pT;
TH1F *Reco_Onia_pt[onia::kNbRapForPTBins+1];
TH1F *Reco_Onia_rap[onia::kNbPTMaxBins+1];
TH1F *Reco_Onia_rapcpm[onia::NchBins+1];
TH1F *Reco_Onia_PU[onia::NchBins+1];
TH1F *Reco_Onia_cpm[onia::kNbPTMaxBins+1];
TH2F *Reco_Onia_rap_cpm;
TH1F *Reco_Onia_CPMPUbinned[onia::NchBins];
TH1F *Reco_Onia_CPMPU_intNCH[25];
TH2F *Reco_Onia_pT_cpm;


double runningsumnPriVtxBins[onia::NchBins][25];
double neventsnPriVtxBins[onia::NchBins+1][25];
double runningsumIntNprivtx[onia::NchBins+1];
double runningsumforstddevIntNprivtx[onia::NchBins+1];
...
void PolData::Loop(int nState, bool rejectCowboys, int FidCuts, bool MC, bool RequestTrigger, bool removeEta0p2_0p3, bool cutDeltaREllDpt) {
	...
	treeOut->Branch("PriVtx", &PriVtx, "PriVtx/D");
	...
	double jpsiVprob = 0;
	double NchRunningSum = 0;
    double NchAvg = 0;
    double NchBinned[26];
    double EventsBinned[26];
    double BinnedNchAvg[26];

	treeOut->Branch("nPriVtx", &nPriVtx, "nPriVtx/D");
	treeOut->Branch("Jpsict", &jpsict, "Jpsict/D");
	treeOut->Branch("vertexWeight", &CPM, "vertexWeight/D");
	...
	//for (Long64_t jentry=3*nentries/4; jentry<nentries; jentry++) {  //  four quarter
		...
		if(nState == 5){
			if(
					...
					HLT_Dimuon5_PsiPrime_v3== 1 || //2012
					HLT_Dimuon5_PsiPrime_v4== 1 || //2012
					HLT_Dimuon5_PsiPrime_v5== 1 || //2012
					HLT_Dimuon5_PsiPrime_v6== 1 || //2012
					HLT_Dimuon9_PsiPrime_v9== 1  //2012
		...
		if(onia_pt > onia::pTRange[0][0] && onia_pt < onia::pTRange[0][1]) Reco_Onia_mass[0][1][0]->Fill(onia_mass);
		if(onia_pt > onia::pTRange[0][1] && onia_pt < onia::pTRange[0][2]) Reco_Onia_mass[0][2][0]->Fill(onia_mass);
		Reco_Onia_rap[0]->Fill(onia_rap);
		Reco_Onia_pt[0]->Fill(onia_pt);
		Reco_Onia_cpm[0]->Fill(onia_cpm);
		Reco_Onia_PU[0]->Fill(nPriVtx);
...
//				Reco_Onia_mass[0][0][iRap+1]->Fill(onia_mass);
			}//if
			...
			for(int iPT = 0; iPT < onia::kNbPTMaxBins; iPT++){	
			  for(int iCPM = 0; iCPM < onia::NchBins; iCPM++){	
				...
				if(onia_pt > onia::pTRange[0][iPT] && onia_pt < onia::pTRange[0][iPT+1] &&
						TMath::Abs(onia_rap) > onia::rapForPTRange[iRap] && TMath::Abs(onia_rap) < onia::rapForPTRange[iRap+1] && onia_cpm > onia::cpmRange[iCPM] && onia_cpm < onia::cpmRange[iCPM+1]){
					Reco_Onia_mass[iCPM+1][iPT+1][iRap+1]->Fill(onia_mass);
				}//if
			  }// iCPM 
			} // iPT
		} // iRap
...
//				Reco_Onia_mass[iPT+1][0]->Fill(onia_mass); 
		...
		for(int iPT = 0; iPT < onia::kNbPTMaxBins; iPT++){	
            if(onia_pt > onia::pTRange[0][iPT] && onia_pt < onia::pTRange[0][iPT+1]){
                Reco_Onia_cpm[iPT+1]->Fill(onia_cpm);
     	    }
   		}
   		
   		for(int iCPM = 0; iCPM < onia::NchBins; iCPM++){
   		if(onia_cpm > onia::cpmRange[iCPM] && onia_cpm < onia::cpmRange[iCPM+1]){
            if(nPriVtx > 0 && nPriVtx < 26){
                Reco_Onia_PU[iCPM+1]->Fill(nPriVtx);
     	    }
     	   }
   		}
