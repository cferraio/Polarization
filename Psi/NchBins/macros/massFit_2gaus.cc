...
//		ws->var("CBsigma2")->setVal(.02);
		ws->var("CBsigma")->setVal(0.021);
		ws->var("CBsigma2")->setVal(0.034);
		ws->var("CBsigma")->setConstant(kTRUE);
		ws->var("CBsigma2")->setConstant(kTRUE);
...
*/
		...
		ws->var("CBsigma")->setVal(3.686/3.097*0.021); //mass of 2s/1s times resolution from 1s. resolution first left as free parameter, then set constant when observed to not depend on Nch or pT
		ws->var("CBsigma2")->setVal(3.686/3.097*0.034);
		ws->var("CBsigma")->setConstant(kTRUE);
		ws->var("CBsigma2")->setConstant(kTRUE);
