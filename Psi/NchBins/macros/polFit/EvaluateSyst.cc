...
int main(int argc, char* argv[]) {
	...
	filename1 << storagedir.c_str() << "/Data/" << JobID1.c_str() << "/TGraphResults_Psi" << nState-3 << "S.root";
	filename2 << storagedir.c_str() << "/Data/" << JobID2.c_str() << "/TGraphResults_Psi" << nState-3 << "S.root";
	...
	for(int iLam = 1; iLam < 19; iLam++){
		...
		for(int ptBin = ptBinMin; ptBin < ptBinMax+1; ptBin++){
			...
			double cpmCentre_[nBinscpm],
						 cpmCentreErr_low[nBinscpm],
						 cpmCentreErr_high[nBinscpm],
						 lmean[nBinscpm],
						 cpmCentre1_[nBinscpm],
						 cpmCentreErr1_low[nBinscpm],
						 cpmCentreErr1_high[nBinscpm],
						 cpmCentre2_[nBinscpm],
						 cpmCentreErr2_low[nBinscpm],
						 cpmCentreErr2_high[nBinscpm],
						 lmean1[nBinscpm],
						 lmean2[nBinscpm],
						 lmeanErr1_low[nBinscpm],
						 lmeanErr1_high[nBinscpm],
						 lmeanErr2_low[nBinscpm],
						 lmeanErr2_high[nBinscpm],
						 value[nBinscpm-1],
			...
			for(int cpmBin = cpmBinMin; cpmBin < cpmBinMax+1; cpmBin++) {
				...
				cpmCentre_[cpm]=(cpmCentre1_[cpm]+cpmCentre2_[cpm])/2.;
				...
				cout<<"cpmCentre1_: "<<cpmCentre1_[cpm]<<endl;
				cout<<"cpmCentre2_: "<<cpmCentre2_[cpm]<<endl;
				cout<<"cpmCentreErr1_low: "<<cpmCentreErr1_low[cpm]<<endl;
				cout<<"cpmCentreErr2_low: "<<cpmCentreErr2_low[cpm]<<endl;
				cout<<"cpmCentreErr1_high: "<<cpmCentreErr1_high[cpm]<<endl;
				cout<<"cpmCentreErr2_high: "<<cpmCentreErr2_high[cpm]<<endl;
				cout<<"cpmCentreErr2_high: "<<cpmCentreErr2_high[cpm]<<endl;
				cout<<"lmean1: "<<lmean1[cpm]<<endl;
				cout<<"lmean2: "<<lmean2[cpm]<<endl;
				...
				//lmean[cpm]=fabs(lmean1[cpm]-lmean2[cpm])/2.;
				//lmean[cpm]=fabs(lmean1[cpm]-lmean2[cpm])/TMath::Sqrt(12);
				//lmean[cpm]=fabs(lmean1[cpm]-lmean2[cpm]);
				//lmean[cpm] = lmean[cpm] * lmean[cpm] ;

				//lmean[cpm] = lambdaLowcpm[iLam-1] ;
//				if(cpm>8){
//					if(lambdaHighest[iLam-1] < lmean[cpm]) lambdaHighest[iLam-1] = lmean[cpm];
					//lmean[cpm] = lambdaHighestVal[iLam-1];
//				}
				...
				//if(lmeanBuff < 0) lmean[cpm]=lmeanBuff/TMath::Sqrt(lmeanErr2_low[cpm]*lmeanErr2_low[cpm]+lmeanErr1_high[cpm]*lmeanErr1_high[cpm]); //ifPull
				//else lmean[cpm]=lmeanBuff/TMath::Sqrt(lmeanErr1_low[cpm]*lmeanErr1_low[cpm]+lmeanErr2_high[cpm]*lmeanErr2_high[cpm]); //ifPull
			...
			if(centralsPlusSyst) {
				...
				//std::cout << "centralsPlusSyst ocpmion" << std::endl;
			}
			// if 'take central value from JobID2, take error from JobID1'
				...
				//std::cout << "differentErrors ocpmion" << std::endl;
			}
