...
*/
			...
			if(PlotCompare){ //not yet changed 12.12.2012 Linlin
				...
				if(nComp>0){//FindLegend
					cpm=0;
					for(int cpmBin = cpmBinMin; cpmBin < cpmBinMax+1; cpmBin++) {
						graphCompareFile1->GetPoint(cpmBin-1,cpmCentre[cpm],lmean[cpm]);
						cpmCentreErr_high[cpm]=graphCompareFile1->GetErrorXhigh(cpmBin-1);
						cpmCentreErr_low[cpm]=graphCompareFile1->GetErrorXlow(cpmBin-1);
						lmean_errhigh[cpm]=graphCompareFile1->GetErrorYhigh(cpmBin-1);
						lmean_errlow[cpm]=graphCompareFile1->GetErrorYlow(cpmBin-1);
						if(ShiftCompareInX){
							cpmCentre[cpm] = cpmCentre[cpm] - DeltaXCompare;
							cpmCentreErr_high[cpm] = cpmCentreErr_high[cpm]+DeltaXCompare; //0.
							cpmCentreErr_low[cpm] = cpmCentreErr_low[cpm]-DeltaXCompare; //0.
						}
						cpm++;
					}
					graphCompareFile1 = new TGraphAsymmErrors(nBinscpm,cpmCentre,lmean,cpmCentreErr_low,cpmCentreErr_high,lmean_errlow,lmean_errhigh);
					graphCompareFile1->SetMarkerColor(kRed);
					graphCompareFile1->SetLineColor(kRed);
					graphCompareFile1->SetMarkerStyle(28);
				...
				if(nComp>1){
					cpm=0;
					for(int cpmBin = cpmBinMin; cpmBin < cpmBinMax+1; cpmBin++) {
						graphCompareFile2->GetPoint(cpmBin-1,cpmCentre[cpm],lmean[cpm]);
						cpmCentreErr_high[cpm]=graphCompareFile2->GetErrorXhigh(cpmBin-1);
						cpmCentreErr_low[cpm]=graphCompareFile2->GetErrorXlow(cpmBin-1);
						lmean_errhigh[cpm]=graphCompareFile2->GetErrorYhigh(cpmBin-1);
						lmean_errlow[cpm]=graphCompareFile2->GetErrorYlow(cpmBin-1);
						if(ShiftCompareInX){
							cpmCentre[cpm] = cpmCentre[cpm] + DeltaXCompare;
							cpmCentreErr_high[cpm] = cpmCentreErr_high[cpm]-DeltaXCompare; //0.
							cpmCentreErr_low[cpm] = cpmCentreErr_low[cpm]+DeltaXCompare; //0.
					...
					graphCompareFile2 = new TGraphAsymmErrors(nBinscpm,cpmCentre,lmean,cpmCentreErr_low,cpmCentreErr_high,lmean_errlow,lmean_errhigh);
					graphCompareFile2->SetMarkerColor(kBlue);
					graphCompareFile2->SetLineColor(kBlue);
					graphCompareFile2->SetMarkerStyle(30);
				...
				if(nComp>2){
					cpm=0;
					for(int cpmBin = cpmBinMin; cpmBin < cpmBinMax+1; cpmBin++) {
						graphCompareFile3->GetPoint(cpmBin-1,cpmCentre[cpm],lmean[cpm]);
						cpmCentreErr_high[cpm]=graphCompareFile3->GetErrorXhigh(cpmBin-1);
						cpmCentreErr_low[cpm]=graphCompareFile3->GetErrorXlow(cpmBin-1);
						lmean_errhigh[cpm]=graphCompareFile3->GetErrorYhigh(cpmBin-1);
						lmean_errlow[cpm]=graphCompareFile3->GetErrorYlow(cpmBin-1);
						if(ShiftCompareInX){
							cpmCentre[cpm] = cpmCentre[cpm] - 2*DeltaXCompare;
							cpmCentreErr_high[cpm] = cpmCentreErr_high[cpm]+2*DeltaXCompare; //0.
							cpmCentreErr_low[cpm] = cpmCentreErr_low[cpm]-2*DeltaXCompare; //0.
						}
						cpm++;
					}
					graphCompareFile3 = new TGraphAsymmErrors(nBinscpm,cpmCentre,lmean,cpmCentreErr_low,cpmCentreErr_high,lmean_errlow,lmean_errhigh);
					graphCompareFile3->SetMarkerColor(kGreen-2);
					graphCompareFile3->SetLineColor(kGreen-2);
					graphCompareFile3->SetMarkerStyle(3);
				...
				if(nComp>3){
					cpm=0;
					for(int cpmBin = cpmBinMin; cpmBin < cpmBinMax+1; cpmBin++) {
						graphCompareFile4->GetPoint(cpmBin-1,cpmCentre[cpm],lmean[cpm]);
						cpmCentreErr_high[cpm]=graphCompareFile4->GetErrorXhigh(cpmBin-1);
						cpmCentreErr_low[cpm]=graphCompareFile4->GetErrorXlow(cpmBin-1);
						lmean_errhigh[cpm]=graphCompareFile4->GetErrorYhigh(cpmBin-1);
						lmean_errlow[cpm]=graphCompareFile4->GetErrorYlow(cpmBin-1);
						if(ShiftCompareInX){
							cpmCentre[cpm] = cpmCentre[cpm] + 2*DeltaXCompare;
							cpmCentreErr_high[cpm] = cpmCentreErr_high[cpm]-2*DeltaXCompare; //0.
							cpmCentreErr_low[cpm] = cpmCentreErr_low[cpm]+2*DeltaXCompare; //0.
					...
					graphCompareFile4 = new TGraphAsymmErrors(nBinscpm,cpmCentre,lmean,cpmCentreErr_low,cpmCentreErr_high,lmean_errlow,lmean_errhigh);
					graphCompareFile4->SetMarkerColor(kOrange);
					graphCompareFile4->SetLineColor(kOrange);
					graphCompareFile4->SetMarkerStyle(5);
