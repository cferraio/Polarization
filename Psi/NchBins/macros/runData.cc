...
void BookHistosReco(int nState){
  ...
  for(int iRap = 0; iRap < onia::kNbRapForPTBins+1; iRap++){
    ...
    for(int iPT = 0; iPT < onia::kNbPTMaxBins+1; iPT++){
      for(int iCPM = 0; iCPM < onia::NchBins+1; iCPM++){
      	//Mass:
      	std::stringstream name;
      	name << "Reco_Onia_mass_rap" << iRap << "_pT" << iPT << "_CPM" << iCPM;
      	std::string title = ";M [GeV]";
      	Reco_Onia_mass[iCPM][iPT][iRap] = new TH1F(name.str().c_str(), title.c_str(), nBinsMass, massMin, massMax);
      	Reco_Onia_mass[iCPM][iPT][iRap]->Sumw2();
      }
    } // iRap
  } // iPT
  ...
  for(int iCPMBin = 0; iCPMBin < onia::NchBins+1; iCPMBin++){  
    sprintf(name, "Reco_Onia_PU_cpm%d", iCPMBin);
    sprintf(title, "Pileup Distribution");
    Reco_Onia_PU[iCPMBin] = new TH1F(name, title, 25, 1, 25);
    Reco_Onia_PU[iCPMBin]->Sumw2();
  }
...
void WriteHistosReco(const std::string &fNameOut){
  ...
  Reco_Onia_mass[0][0][0]->Write();
  
  for(int iRap = 1; iRap < onia::kNbRapForPTBins+1; iRap++){
  	Reco_Onia_pt[iRap]->Write();
  	
    for(int iPT = 1; iPT < onia::kNbPTMaxBins+1; iPT++){  
      for(int iCPM = 1; iCPM < onia::NchBins+1; iCPM++){    
        Reco_Onia_mass[iCPM][iPT][iRap]->Write();
      }
    }
  }
