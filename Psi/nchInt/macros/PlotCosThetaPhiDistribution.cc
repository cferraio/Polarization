...
TH1D* CosThDistRunNb[10000][onia::kNbRapForPTBins+1][onia::kNbPTMaxBins+1];
TGraphErrors *hSR_cosThetarunNb[onia::kNbRapForPTBins+1][onia::kNbPTMaxBins+1];;
...
//////////////////////////////
//////////////////////////////
//determination of run numbers and initialization of run number graphs
//bool dorunnums = true;
//if(dorunnums)
//{
   Char_t name[100];
   sprintf(name, "%s/tmpFiles/selEvents_data.root", DataPath);
...
//   for(int i=0;i<20000;i++){
   	...
   	long iEntry = selectedData->LoadTree(i);
   	
	   selectedData->GetEvent( iEntry );
	   
	   if(i == 0)  runnumbers.push_back (runnb); 
		   
		   		if(i % 100000 == 0) std::cout << "entry " << i << " out of " << selectedData->GetEntries() << std::endl;
	  ...
	  runnumarray[x] = runnumbers[x];	   
		for(int iRap = 0; iRap < onia::kNbRapForPTBins; iRap++){
			for(int iPT = 0; iPT < onia::kNbPTBins[iRap+1]; iPT++){
			   CosThDistRunNb[x][iRap][iPT] = new TH1D("","",nBins1DCosth, onia::cosTMin, onia::cosTMax);
			   CosThDistRunNb[x][iRap][iPT]->Sumw2();
		   }
	   }
   }
//}
	...
	for(int iRap = 0; iRap < onia::kNbRapForPTBins; iRap++){
		...
		for(int iPT = 0; iPT < onia::kNbPTBins[iRap+1]; iPT++){
			for(int iFrame = 0; iFrame < onia::kNbFrames; iFrame++){
				...
				Char_t name[100];
				sprintf(name, "%s/tmpFiles/data_Psi%dS_rap%d_pT%d.root", DataPath,nState-3, iRap+1, iPT+1);
				TFile *fIn = new TFile(name);
				...
				CosThDist[iFrame][iRap][iPT]->Sumw2();
				PhiDist[iFrame][iRap][iPT]->Sumw2();
...
//			double runnumarray[sz];
	...
	char histName[200];
	double avgCosTh[sz];
	double avgCosTherror[sz];
	double newrunnumarray[sz];
	...
	for(int iRap = 0; iRap < onia::kNbRapForPTBins; iRap++){
		for(int iPT = 0; iPT < onia::kNbPTBins[iRap+1]; iPT++){
			for ( int x=0;  x < sz;  x++ ){
				newrunnumarray[x]=runnumarray[x];
				avgCosTh[x] = CosThDistRunNb[x][iRap][iPT]->GetMean();
				cout<<"mean is"<<avgCosTh[x]<<endl;
				avgCosTherror[x] = CosThDistRunNb[x][iRap][iPT]->GetMeanError();
				xer[x]=0;
			}
	
			AngDistHistFile->cd();
			//sprintf(histName,"Proj_PX_costh_rap%d_pT%d_run%d",iRap+1,iPT+1,runnum);
			sprintf(histName,"Proj_PX_costhRunNb_rap%d_pT%d",iRap+1,iPT+1);
			//CosThDistRunNb[x][iRap][iPT]->SetName(histName);
			//CosThDistRunNb[x][iRap][iPT]->Write();
			cout<<sz<<"here"<<endl;
			hSR_cosThetarunNb[iRap][iPT] = new TGraphErrors(sz, newrunnumarray, avgCosTh, xer, avgCosTherror);
			hSR_cosThetarunNb[iRap][iPT]->SetMarkerStyle(20);
			hSR_cosThetarunNb[iRap][iPT]->Draw("ap");
			hSR_cosThetarunNb[iRap][iPT]->Write(histName);
		}
	}
	...
	for(int iRap = 0; iRap < onia::kNbRapForPTBins; iRap++){
		for(int iFrame = 0; iFrame < 3; iFrame++){
			for(int iPT = 0; iPT < onia::kNbPTBins[iRap+1]; iPT++){
				...
				sprintf(histName,"Proj_%s_phi_rap%d_pT%d",onia::frameLabel[iFrame],iRap+1,iPT+1);
...
void PlotHistos(int iRapBin, int iPTBin, int iFrame,int nState, Char_t *DataPath){
	...
	Char_t name[500];
	sprintf(name, "c1_%s_rap%d_pT%d", onia::frameLabel[iFrame], iRapBin+1, iPTBin+1);
	...
	bool runPlots = true;
	bool saveRunPlots = false;
	if(runPlots && iFrame==2) //2 for px
	{
		for ( int x=0;  x < sz;  x++ ){
				runnum=runnumarray[x];
				sprintf(costhpx,"%s/Figures/costh%s_rap%d_pT%d_runNum%d.pdf",DataPath,onia::frameLabel[iFrame], iRapBin+1, iPTBin+1,runnum);
				CosThDistRunNb[x][iRapBin][iPTBin]->Draw();
				if(iRapBin==0)
				latex->DrawLatex(left,top,Form("|y| < %.1f, %.1f < p_{T} < %.1f GeV",
				onia::rapForPTRange[iRapBin+1],
				onia::pTRange[iRapBin+1][iPTBin],onia::pTRange[iRapBin+1][iPTBin+1]));
				...
				onia::rapForPTRange[iRapBin],onia::rapForPTRange[iRapBin+1],
				onia::pTRange[iRapBin+1][iPTBin],onia::pTRange[iRapBin+1][iPTBin+1]));
				if(saveRunPlots) c1->SaveAs(costhpx);
		 }
...
//	for(int x = 0; x < sz; x++){
//		CosThDistRunNb[x][iRapBin][iPTBin] = new TH1D("","",nBins1DCosth, onia::cosTMin, onia::cosTMax);
//	}
	...
	for(int i=0;i<selectedData->GetEntries();i++){
		...
		for ( int x=0;  x < sz;  x++ ){
			 if(runnumarray[x] == runnb) {
				CosThDistRunNb[x][iRapBin][iPTBin]->Fill(TMath::Abs(costh_PX));
			 }
		 }
