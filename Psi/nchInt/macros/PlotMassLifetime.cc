...
void plotLifeBgIndividual(RooWorkspace *ws, int rapBin, int ptBin, int nState);
void plotLifeSigIndividual(RooWorkspace *ws, int rapBin, int ptBin, int nState);
//==============================================
...
//			plotMassLog(ws, rapBin, ptBin, nState);
		...
		case 5:
			...
			plotLifeBgIndividual(ws, rapBin, ptBin, nState);
			break;
		case 6:
			std::cout << ">>>>Plotting lifetime signal region, separate pull and distribution" << std::endl;
			plotLifeSigIndividual(ws, rapBin, ptBin, nState);
			break;
		default:
...
void plotLifeBg(RooWorkspace *ws, int rapBin, int ptBin, int nState){
	...
	savectLSB << "Fit/ct_LSB_rap" << rapBin << "_pt" << ptBin << "_withPull.pdf";
	...
	savectRSB << "Fit/ct_RSB_rap" << rapBin << "_pt" << ptBin << "_withPull.pdf";
...
void plotLifeSig(RooWorkspace *ws, int rapBin, int ptBin, int nState){
	...
	//savectlog << "Fit/ct_SR_log_rap" << rapBin << "_pt" << ptBin << "_pullAverage_withPull.pdf";
	savectlog << "Fit/ct_SR_log_rap" << rapBin << "_pt" << ptBin << "_withPull.pdf";
...
void plotLifeSig_linear(RooWorkspace *ws, int rapBin, int ptBin, int nState){
	...
	savect << "Fit/ct_SR_rap" << rapBin << "_pt" << ptBin << "_withPull.pdf";
...
void plotLifeBgIndividual(RooWorkspace *ws, int rapBin, int ptBin, int nState){
	...
	else pullLatexLSB << Form("%.1f < p_{T} < %.1f GeV",onia::pTRange[rapBin][ptBin-1],onia::pTRange[rapBin][ptBin]);

	latex->DrawLatex(left,top,pullLatexLSB.str().c_str());

	std::stringstream savectLSBPull;
	savectLSBPull << "Fit/ct_LSB_rap" << rapBin << "_pt" << ptBin << "_pull.pdf";
	...
	savectRSBPull << "Fit/ct_RSB_rap" << rapBin << "_pt" << ptBin << "_pull.pdf";
...
void plotLifeSigIndividual(RooWorkspace *ws, int rapBin, int ptBin, int nState){
	...
	savectlogPull << "Fit/ct_SR_log_rap" << rapBin << "_pt" << ptBin << "_pull.pdf";
