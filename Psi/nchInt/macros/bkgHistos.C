...
//---------------------------------------------------------------------------------------------------------------
void bkgHistos(const std::string infilename, int rapBin, int ptBin, int nState, bool folding, bool MC, bool doCtauUncer, bool PolLSB, bool PolRSB, bool PolNP, int ctauScen, int FracLSB, bool forceBinning, bool normApproach, bool scaleFracBg, char *polDataPath, bool officialMC){
if(officialMC) cout<<"official mc"<<endl;
if(!officialMC) cout<<"not official mc"<<endl;
	const std::string
...
//		hSR_cosThetarunNb[iFrame]->Sumw2();
  ...
  //double fBkgLSBtotInP = InteRltsSB[2];
	...
	if(MC || PolLSB || PolRSB || officialMC){
		fBGinNP = 0.001;
		fP      = 0.999;
		fNPB    = 0.001;
		fBGsig  = 0.001;
	...
	if(!MC && !PolLSB && !PolRSB && !PolNP && !officialMC){
		if(doCtauUncer){
			int nEvents = 50;
