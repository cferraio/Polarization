...
//	TH2D *hSR_cosThetarunNb[onia::kNbFrames];
cout<<"inbkghistos3"<<endl;	

	for(int iFrame = 0; iFrame < onia::kNbFrames; iFrame++){
		//book the 2D (cosTheta, phi) histos for the L and R mass sideband
		std::stringstream nameL, nameR, nameNP, nameBGinNPL, nameBGinNPR, nameSRL, nameSRR, nameSR, title, namecosthrunNb, titlecosthrunNb;
		...
		namecosthrunNb << "hSR_cosThetarunNb_" << onia::frameLabel[iFrame];
		title << ";cos#theta_{"<< onia::frameLabel[iFrame] << "};#phi_{" << onia::frameLabel[iFrame] << "} [deg]";
		titlecosthrunNb << ";cos#theta_{"<< onia::frameLabel[iFrame] << "};Run Number";
...
cout<<"inbkghistos4"<<endl;		
//		hSR_cosThetarunNb[iFrame] = new TH2D(namecosthrunNb.str().c_str(), title.str().c_str(),
//				onia::kNbBinsCosT, onia::cosTMin, onia::cosTMax, 1850, 190500, 209000);
//		hSR_cosThetarunNb[iFrame]->Sumw2();
...
//						hSR_cosThetarunNb[iFrame]->Fill(thisCosTh[iFrame], runnumber);
					}
				}
	...
	for(int iFrame = 0; iFrame < onia::kNbFrames; iFrame++){
		//book the 2D (cosTheta, phi) histos for the L and R mass sideband
		std::stringstream nameL, nameR, nameNP, nameBGinNPL, nameBGinNPR, namecosthrunNb, nameSR, title, titlecosthrunNb;
		nameL << "hBG_cosThetaPhi_" << onia::frameLabel[iFrame] << "_L";
		nameR << "hBG_cosThetaPhi_" << onia::frameLabel[iFrame] << "_R";
		nameNP << "hNPBG_cosThetaPhi_" << onia::frameLabel[iFrame];
		nameBGinNPL << "hBGinNP_cosThetaPhi_" << onia::frameLabel[iFrame] << "_L";
...
//	long iEntry = intree->LoadTree(1);
//	intree->GetEntry(iEntry);
//	runnumbers.push_back(runnb);
//	cout<<"here"<<runnumbers[0];
	
	for(int i = 0; i < n; i++){
		long iEntry = intree->LoadTree(i);
		intree->GetEntry(iEntry);
//		if(i % 100000 == 0) std::cout << "run number  " << i << " is " << runnb << std::endl;
		...
		{ 
			...
			if(runnb != runnumbers[val-1] && val == sz) runnumbers.push_back (runnb); 
		}
		
	}
	
//	sz = runnumbers.size();
	double runnumarray[sz];
	for(int x = 0; x < sz; x++){
		runnumarray[x] = runnumbers[x];
//		cout<<runnumarray[x]<<endl;
	}
	

	
	int smallest = runnumarray[0] ;
    for ( int i=0;  i < sz;  i++ ){
        if ( runnumarray[i] < smallest )
             smallest = runnumarray[i];
    }
             
	int largest = runnumarray[0] ;
    for ( int i=0;  i < sz;  i++ ){
        if ( runnumarray[i] > largest )
             largest = runnumarray[i];
    }
    
    double range = largest - smallest;
	

//	TH1D *hSR_cosThetarunNb= new TProfile("hprof","Profile of pz versus px",100,-4,4,0,20);
	
//	std::stringstream titlecosthrunNb;
//	TH1D *hSR_cosThetarunNb[sz];
//	char namecosthrunNb[200];
//	delete hSR_cosThetarunNb[iFrame];	
/*	for(int x = 0; x < sz; x++)
	{
//	    namecosthrunNb << "hSR_cosThetaPX_runNb" << runnumarray[x];
//	    titlecosthrunNb << ";cos#theta_PX";
		sprintf(namecosthrunNb,"hSR_cosTheta_runNb%d",runnumarray[x]);
		hSR_cosThetarunNb[x] = new TH1D(namecosthrunNb, titlecosthrunNb.str().c_str(),
			onia::kNbBinsCosT, onia::cosTMin, onia::cosTMax);
		hSR_cosThetarunNb[x]->Sumw2();
	}	
*/	/////////here///////

double avgCosTh[sz];
double eventnumber[sz];
double totalCosTh[sz];
		...
		if(jpsi->Pt() >= onia::pTRange[rapBin-1][ptBin-1] &&
			...
			for(int iFrame = 0; iFrame < onia::kNbFrames; iFrame++){
				...
				if(MC){
					...
					if(index == 5 && iFrame == 2)	{//for(int x = 0; x < sz; x++){
//							 hSR_cosThetarunNb[iFrame]->Fill(thisCosTh[iFrame]);
							 for ( int x=0;  x < sz;  x++ ){
								if(runnumarray[x] == runnumber) {
								  eventnumber[x]++;
								  
								  totalCosTh[x] = totalCosTh[x]+TMath::Abs(thisCosTh[iFrame]);
								 // avgCosTh[sz] = totalCosTh[sz]/eventnumber[sz]
								}
					 		}
					//	}
//						cout<<runnumber<<" is the run number"<<endl;
					}
				}
			} // iFrame
	...
	cout<<"average cos"<<endl;
			for ( int x=0;  x < sz;  x++ ){
				avgCosTh[x] = totalCosTh[x]/eventnumber[x];
				cout<<avgCosTh[x]<<endl;
				
//				hSR_cosThetarunNb->Fill(runnumarray[x],avgCosTh[x]);
			}
			
				TGraph *hSR_cosThetarunNb;
				hSR_cosThetarunNb = new TGraph(sz, runnumarray, avgCosTh);
				hSR_cosThetarunNb->SetMarkerStyle(20);
	...
	//======================================================
	hSR_cosThetarunNb->Write("hSR_cosThetarunNb");
