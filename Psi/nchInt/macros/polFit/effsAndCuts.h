...
bool isMuonInAcceptance(int iCut, double pT, double eta){
	...
	if(iCut==15){
		if(TMath::Abs(eta)<1.2 && pT>5) decision=kTRUE;
		if(TMath::Abs(eta)>1.2 && TMath::Abs(eta)<1.4 && pT>4) decision=kTRUE;
		if(TMath::Abs(eta)>1.4 && TMath::Abs(eta)<1.6 && pT>3.5) decision=kTRUE;
	}
	
	if(iCut==16){
		if(TMath::Abs(eta)<1.2 && pT>6) decision=kTRUE;
		if(TMath::Abs(eta)>1.2 && TMath::Abs(eta)<1.4 && pT>5) decision=kTRUE;
		if(TMath::Abs(eta)>1.4 && TMath::Abs(eta)<1.6 && pT>4.5) decision=kTRUE;
	}
	
	if(iCut==17){ //
		if(pT>5.0) decision=kTRUE;
	}
	
	if(iCut==18){
		if(pT>5.5) decision=kTRUE;
	}
	
	if(iCut==19){
		if(pT>6.0) decision=kTRUE;
	}
	
	if(iCut==20){
		if(pT>5.25) decision=kTRUE;
	}
	
	if(iCut==21){
		if(pT>5.75) decision=kTRUE;
	}
	
	if(iCut==22){
		if(pT>4.5) decision=kTRUE;
	}
	
	if(iCut==23){
		if(pT>4.75) decision=kTRUE;
	}
...
//		if(nEff==1067) sprintf(EffFileName,"ParametrizedFactDataEff_2016_01_14_Central.root"); //2012 eff implemented **WITHOUT** interpolation
		
		if(nEff==1067) sprintf(EffFileName,"singleMuonEff_L3ptg2_seagulls_final.root"); //2012 eff implemented **WITHOUT** parametrization
	...
	if(!singleLeptonEff) {
		...
		if(nEff==330) sprintf(EffFileName,"rho_factor_histograms.root");
	}
	
if(nEff==100001) sprintf(EffFileName,"singleMuonEff_noTracking_L3ptg2_final.root");

if(nEff==100002) sprintf(EffFileName,"ParametrizedFactDataEff_2016_01_14_Central.root"); //2012 eff
		
if(nEff==100003) sprintf(EffFileName,"singleMuonEff_L3ptg2_seagulls_final.root"); //newest 2012 eff
...
double evalParametrizedEff(double &pT, double &eta, TGraphAsymmErrors *func){
...
double EvaluateRhoFactor( double& costh, double& phi, int nEff, TFile* fInRhoFactor, double rap, double pT, bool StatVarRho) {
	...
	int pTbin;
	int rapBin;
	//const int nRhoPtBins=16; //2011 rho number
	const int nRhoPtBins=13; //2012 rho number
	...
	//Double_t pTRangeRho[nRhoPtBins+1] = {10.,12.,14.,16.,18.,20.,22.,25.,30.,35.,40.,45.,50.,55.,60.,65.,70.}; //2011 
	...
	if(nEff>300){
		...
		if(nEff>320 && nEff<330) sprintf(EffType,"rho_eff_MCtruth_PHX_pT%d_rap%d",pTbin,rapBin);
		if(nEff==330) sprintf(EffType,"rho_eff_MCtruth_PHX_pt%d_rap%d",pTbin,rapBin);
