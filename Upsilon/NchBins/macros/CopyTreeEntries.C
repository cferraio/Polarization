...
void CopyTreeEntries(Int_t iRapBin = 1, 
		     Int_t iPTBin = 1,
		     Int_t iCPMBin = 1,
		     Char_t *fileNameIn = "RootFiles/selEvents_data_Ups.root",
		     bool UpsMC=false,
		     bool DoCPUconsumingPlots=false){

  Char_t name[100], title[100];  
  Char_t fileNameOut[100];
  sprintf(fileNameOut, "tmpFiles/data_Ups_rap%d_pT%d_cpm%d.root", iRapBin, iPTBin, iCPMBin);
  ...
  TTree *treein = (TTree *) gDirectory->Get("CPM");
...
/////////////////
/////////////////
  if(iRapBin==0&&iPTBin==0&&iCPMBin==0&&PlotSpecials){
	  char CutChar[200];
	  char savename[200];
	  TCanvas *c2;
	  TCanvas *c3;
	  ...
	  TH1D* dimumasspT1   = new TH1D( "dimumasspT1", "dimumasspT1", DMnBins1, DMmin,DMmax);
	  TH1D* dimumasspT2   = new TH1D( "dimumasspT2", "dimumasspT2", DMnBins2, DMmin,DMmax);
...
//		TLegend* DMLegend=new TLegend(0.53,0.75,0.95,0.9);
		TLegend* DMLegend=new TLegend(0.58,0.585,0.95,0.725);
...
//		DMLegend->SetMargin(0.135);
	  ...
	  double rapMin=0;
	  double rapMax=1.2;
	  sprintf(DMDrawChar,"%s>>dimumasspT1",DimuonMassChar);
	  ...
	  rapMin=0;
	  rapMax=1.2;
	  sprintf(DMDrawChar,"%s>>dimumasspT2",DimuonMassChar);
	  sprintf(DMCutChar,"%s>%f && %s<%f && %s>%f && %s<%f",PtChar,ptMin,PtChar,ptMax,AbsRapChar,rapMin,AbsRapChar,rapMax);
      ...
      gPad->SetTopMargin(MargTop);
 	  ...
 	  dimumasspT1->SetYTitle(DMyTitle);
	  dimumasspT1->SetXTitle("Dimuon mass [GeV]");
	  dimumasspT1->SetTitle(0);
	  dimumasspT1->SetStats(0);
	  dimumasspT1->GetYaxis()->SetTitleOffset(1.25);
	  dimumasspT1->SetMarkerStyle(25);
	  dimumasspT1->SetMarkerSize(MarkerSizeTom);
	  dimumasspT1->SetMarkerColor(kBlack);
	  dimumasspT1->SetLineColor(kBlack);
	  dimumasspT2->SetTitle(0);
	  dimumasspT2->SetStats(0);
	  dimumasspT2->SetMarkerColor(kRed);
	  dimumasspT2->SetMarkerStyle(20);
	  dimumasspT2->SetLineColor(kRed);
	  dimumasspT2->SetMarkerSize(MarkerSizeTom);
...
//	        	int globalBin=dimumasspT1->GetBin(iX);
	  ...
	  dimumasspT1->Draw("E1");
	  dimumasspT2->Draw("same,E1");
	  dimumasspT1->Draw("same,chist");
	  dimumasspT2->Draw("same,chist");


//		sprintf(DMLegendEntry,"|#it{y}| < 0.6, per %d MeV",int(1000*DMbinwidth1));
		sprintf(DMLegendEntry,"10 < p_{T} < 15 GeV");
		DMLegend->AddEntry(dimumasspT1,DMLegendEntry,"p");
//		sprintf(DMLegendEntry,"0.6 < |#it{y}| < 1.2, per %d MeV",int(1000*DMbinwidth2));
		sprintf(DMLegendEntry,"15 < p_{T} < 50 GeV");
		DMLegend->AddEntry(dimumasspT2,DMLegendEntry,"p");
	 ...
	 cout<<text<<endl;
	 TLatex *CentralsText1DM = new TLatex(xCentralsDM+xCentralsDM_shift,dimumasspT1->GetMaximum()*0.975,text);
	 CentralsText1DM->SetTextSize(CentralsFontSize);
	 CentralsText1DM->Draw( "same" );
	 sprintf(text,"L = 4.9 fb^{-1}");
	 cout<<text<<endl;
	 TLatex *CentralsText2DM = new TLatex(xCentralsDM+xCentralsDM_shift,dimumasspT1->GetMaximum()*0.825,text);
//	 if(!CMSprelim) CentralsText2DM = new TLatex(xCentralsDM+0.15,dimumassRap2->GetMaximum()*0.65,text);
	 ...
	 cout<<text<<endl;
	 TLatex *CentralsText3DM = new TLatex(xCentralsDM+xCentralsDM_shift,dimumasspT1->GetMaximum()*0.9,text);
//	 if(!CMSprelim) CentralsText3DM = new TLatex(xCentralsDM+0.15,dimumassRap2->GetMaximum()*0.725,text);
	 ...
	 CentralsText3DM->Draw( "same" );
	   ...
	   char abcdef[200];
	   sprintf(abcdef,"a)");
	   TLatex *tex_abcdef = new TLatex(8.95,dimumasspT2->GetMaximum()*0.95,abcdef);
	   tex_abcdef->SetTextSize(CentralsFontSize*1.25);
//	   tex_abcdef->Draw( "same" );

	   sprintf(abcdef,"x 10^{3}");
	   tex_abcdef = new TLatex(8.5,dimumasspT1->GetMaximum()*1.07,abcdef);
...
//      ptRap1->GetYaxis()->SetTitleOffset(1.75);
	  ...
	  ptRap2->Draw("same,E1");
		...
		TLine *PtLine;
		  for(int iPt=0;iPt<onia::kNbPTBins[1]+1;iPt++){
...
//	   tex_abcdef->Draw( "same" );
  ...
  TH2D* PtCPM   = new TH2D( "Ptcpm", "Ptcpm", 110,5,55,60,0,65);
...
//  gPad->SetTopMargin(0.1);
  ...
  for(int iPt=0;iPt<onia::kNbPTBins[iRap+1]+1;iPt++){
    for(int iCPM=0;iCPM<onia::NXBINS+1;iCPM++){
  rapPtLine= new TLine( -onia::rapForPTRange[onia::kNbRapForPTBins], onia::pTRange[0][iCPM], onia::rapForPTRange[onia::kNbRapForPTBins], onia::pTRange[0][iCPM] );
  rapPtLine->SetLineWidth( 2 );
  rapPtLine->SetLineStyle( 1 );
  rapPtLine->SetLineColor( kWhite );
  rapPtLine->Draw();
...
treeIn->Draw("CPM:TMath::Sqrt((lepP->Px()+lepN->Px())*(lepP->Px()+lepN->Px())+(lepP->Py()+lepN->Py())*(lepP->Py()+lepN->Py()))>>Ptcpm",CutChar,"colz");

  c3 = new TCanvas("c3","c3",1200,1500);
  PtCPM->SetYTitle("N_{ch}");

  PtCPM->SetXTitle("#it{p}_{T}(#mu#mu) [GeV]");
  gStyle->SetPalette(1);
  gPad->SetFillColor(kWhite);
  PtCPM->SetTitle(0);
  PtCPM->SetStats(0);
  ...
  for(int iPt=0;iPt<onia::kNbRapForPTBins+1;iPt++){

	  PtcpmLine= new TLine( onia::pTRange[0][iPt], onia::cpmRange[0], onia::pTRange[0][iPt], onia::cpmRange[onia::NXBINS] );

	  PtcpmLine->SetLineWidth( 2 );
	  PtcpmLine->SetLineStyle( 1 );
	  PtcpmLine->SetLineColor( kWhite );
	  PtcpmLine->Draw();
//	  PtcpmLine= new TLine( onia::pTRange[0][iPt], onia::cpmRange[0], onia::pTRange[0][iPt], onia::cpmRange[onia::kNbPTBins[iPt]] );
  ...
  for(int iCPM=0;iCPM<onia::NXBINS+1;iCPM++){
  PtcpmLine= new TLine( onia::pTRange[0][0], onia::cpmRange[iCPM], onia::pTRange[0][onia::kNbPTMaxBins], onia::cpmRange[iCPM] );
  PtcpmLine->SetLineWidth( 2 );
  PtcpmLine->SetLineStyle( 1 );
  PtcpmLine->SetLineColor( kWhite );
  PtcpmLine->Draw();
  }}
  sprintf(savename,"Figures/PtCPM_Ups%dS.pdf",iState);
  c3->SaveAs(savename);
////////////////////////////////
...
/////////////////
  ...
  for(int iEn = 0; iEn < treeIn->GetEntries(); iEn++){ 
    ...
    double ProjectCPMMin;
    double ProjectCPMMax;
    
    if(iCPMBin==0){
    	ProjectCPMMin=onia::cpmRange[0];
    	ProjectCPMMax=onia::cpmRange[onia::NXBINS];
    }
    
    if(iCPMBin>0){
    	ProjectCPMMin=onia::cpmRange[iCPMBin-1];
    	ProjectCPMMax=onia::cpmRange[iCPMBin];
    }
...
//    cout<<"cpm = "<<CPM<<endl;
    *onia = *(lepP) + *(lepN);
    if(CPM > ProjectCPMMin && CPM < ProjectCPMMax && onia->Pt() > ProjectPtMin 
    && onia->Pt() < ProjectPtMax &&
