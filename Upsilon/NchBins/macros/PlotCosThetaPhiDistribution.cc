...
Int_t const cpm_binsFOR1Dhists=onia::NXBINS;
Int_t const Rap_binsFOR1Dhists=onia::kNbRapForPTBins;

TH2D* CosThPhDist[onia::kNbFrames][onia::kNbRapForPTBins+1][onia::kNbPTMaxBins+1][onia::NXBINS+1];
TH1D* CosThDist[onia::kNbFrames][onia::kNbRapForPTBins+1][onia::kNbPTMaxBins+1][onia::NXBINS+1];
TH1D* PhiDist[onia::kNbFrames][onia::kNbRapForPTBins+1][onia::kNbPTMaxBins+1][onia::NXBINS+1];
...
void PlotHistos(Int_t iRapBin, Int_t iPTBin, Int_t iCPMBin, Int_t iFrame, int nState, Char_t *DataPath);
//===========================
int main(int argc, char** argv){
  ...
  sprintf(filename, "%s/Figures/Ups%dS/AngDistHist.root", DataPath,nState);
      ...
      char histName[200];
      for(int iFrame = 0; iFrame < 3; iFrame++){
          sprintf(histName,"Proj_%s_costh_rap%d_pT%d_cpm%d",onia::frameLabel[iFrame],iRap,iPT,iCPM);
          CosThDist[iFrame][iRap][iPT][iCPM]->SetName(histName);
          CosThDist[iFrame][iRap][iPT][iCPM]->Write();
          sprintf(histName,"Proj_%s_phi_rap%d_pT%d_cpm%d",onia::frameLabel[iFrame],iRap,iPT,iCPM);
          PhiDist[iFrame][iRap][iPT][iCPM]->SetName(histName);
          PhiDist[iFrame][iRap][iPT][iCPM]->Write();
          sprintf(histName,"CosthPhi_%s_rap%d_pT%d_cpm%d",onia::frameLabel[iFrame],iRap,iPT,iCPM);
...
//	  gStyle->SetPadLeftMargin(0.12);
  ...
  Char_t name[100];
  sprintf(name, "c1_%s_rap%d_pT%d_cpm%d", onia::frameLabel[iFrame], iRapBin, iPTBin, iCPMBin);
  ...
  sprintf(name, "%s/Figures/Ups%dS/cosThetaPhi_%s_rap%d_pT%d_cpm%d.pdf", DataPath,nState,onia::frameLabel[iFrame], iRapBin, iPTBin, iCPMBin);
  cout<<name<<endl;
  c1->Print(name);
}
