...
then
./runData rejectCowboys=${rejectCowboys} ${inputTree1}=inputTree1 ${inputTree2}=inputTree2 ${inputTree3}=inputTree3 ${inputTree4}=inputTree4 ${FidCuts}FidCuts UpsMC=${UpsMC} RequestTrigger=${RequestTrigger} selectSOFT=${selectSOFT} selectTIGHT=${selectTIGHT} selectMIXED=${selectMIXED} selectNOTMIXED=${selectNOTMIXED} Odd=${Odd} Even=${Even}
fi
