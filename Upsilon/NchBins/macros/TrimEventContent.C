...
double TrimEventContent(Int_t iRapBin = 1,
  ...
  printf("\n\n\nfracL = %1.3f, nSigma = %1.1f, iState = %d, rap %d, pT %d, cpm %d\n\n", fracL, nSigma, nUpsState, iRapBin, iPTBin, iCPMBin);

  Char_t name[100], title[100];
  Char_t fileNameIn[100];
  sprintf(fileNameIn, "tmpFiles/data_Ups_rap%d_pT%d_cpm%d.root", iRapBin, iPTBin, iCPMBin);
  //==============================
  ...
  sprintf(fileNameOut, "AllStates_%1.2fSigma_FracLSB%dPercent/data_%dSUps_rap%d_pT%d_cpm%d.root", nSigma, int(fracL*100), nUpsState+1, iRapBin, iPTBin, iCPMBin);
...
//  Double_t nBGerror = fBG->IntegralAndError(massMin, massMax);
...
//  Double_t nSignalerror = sqrt( pow(fUps[0]->IntegralAndError(massMin,MassMax),2) + pow(fUps[1]->IntegralAndError(massMin, massMax),2) + pow(fUps[2]->IntegralAndError(massMin, massMax),2));
  Double_t fracBG = nBG / (nBG + nSignal);
//  Double_t fracBGerror = fracBG* sqrt(  pow(sqrt(pow(nSignalerror,2)+pow(nBGerror,2))/(nBG+nSignal),2) + pow(nBGerror/nBG,2) ) ;
...
//  cout<<"fracBGerror for rap"<<iRapBin<< " pT "<<iPTBin<<" cpm "<<iCPMBin <<" is "<<fracBGerror<<endl;
...
//    exit(0);
  ...
  if(nSigma==1){
          if(nUpsState==0){
                  massMin=massMinUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMax=massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
          }
          if(nUpsState==1){
                  massMin=massMinUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMax=massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
          }
          if(nUpsState==2){
                  massMin=massMinUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMax=massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
          }
  }

  if(nSigma==3){
          if(nUpsState==0){
                  massMin=massMinUps1S_3sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMax=massMaxUps1S_3sigma[iRapBin][iPTBin-1][iCPMBin-1];
          }
          if(nUpsState==1){
                  massMin=massMinUps2S_3sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMax=massMaxUps2S_3sigma[iRapBin][iPTBin-1][iCPMBin-1];
          }
          if(nUpsState==2){
                  massMin=massMinUps3S_3sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMax=massMaxUps3S_3sigma[iRapBin][iPTBin-1][iCPMBin-1];
          }
  }

  if(nSigma==0.5){
          if(nUpsState==0){
                  massMin=massMinUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]+(massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/4.;
                  massMax=massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/4.;
          }
          if(nUpsState==1){
                  massMin=massMinUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]+(massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/4.;
                  massMax=massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/4.;
          }
          if(nUpsState==2){
                  massMin=massMinUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]+(massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/4.;
                  massMax=massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/4.;
          }
  }

  if(nSigma==0.1){
          if(nUpsState==0){
                  massMin=massMinUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]+(massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/20.;
                  massMax=massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/20.;
          }
          if(nUpsState==1){
                  massMin=massMinUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]+(massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/20.;
                  massMax=massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/20.;
          }
          if(nUpsState==2){
                  massMin=massMinUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]+(massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/20.;
                  massMax=massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/20.;
          }
  }
  ...
  if(nSigma==9){
          if(nUpsState==0){
                  massMin=massMinUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMax=massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/2.;
          }
          if(nUpsState==1){
                  massMin=massMinUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMax=massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/2.;
          }
          if(nUpsState==2){
                  massMin=massMinUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMax=massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/2.;
          }
  }
  ...
  if(nSigma==10){
          if(nUpsState==0){
                  massMax=massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMin=massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps1S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/2.;
          }
          if(nUpsState==1){
                  massMax=massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMin=massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps2S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/2.;
          }
          if(nUpsState==2){
                  massMax=massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1];
                  massMin=massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-(massMaxUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1]-massMinUps3S_1sigma[iRapBin][iPTBin-1][iCPMBin-1])/2.;
          }
  }
