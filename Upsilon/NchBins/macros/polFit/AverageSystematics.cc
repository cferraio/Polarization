...
//ifChange			sprintf(filename,"%s/macros/polFit/Systematics/%s/%s_10B/TGraphResults_%dSUps.root",basedir,SystID,JobID1,nState);
		...
		if(iLam==1)  sprintf(GraphName,"lth_CS_rap1_pt%d",ptBin);
		if(iLam==2)  sprintf(GraphName,"lph_CS_rap1_pt%d",ptBin);
		if(iLam==3)  sprintf(GraphName,"ltp_CS_rap1_pt%d",ptBin);
		if(iLam==6)  sprintf(GraphName,"ltilde_CS_rap1_pt%d",ptBin);

		if(iLam==7)  sprintf(GraphName,"lth_HX_rap1_pt%d",ptBin);
		if(iLam==8)  sprintf(GraphName,"lph_HX_rap1_pt%d",ptBin);
		if(iLam==9)  sprintf(GraphName,"ltp_HX_rap1_pt%d",ptBin);

		if(iLam==12) sprintf(GraphName,"ltilde_HX_rap1_pt%d",ptBin);

		if(iLam==13) sprintf(GraphName,"lth_PX_rap1_pt%d",ptBin);
		if(iLam==14) sprintf(GraphName,"lph_PX_rap1_pt%d",ptBin);
		if(iLam==15) sprintf(GraphName,"ltp_PX_rap1_pt%d",ptBin);

		if(iLam==18) sprintf(GraphName,"ltilde_PX_rap1_pt%d",ptBin);
...
/*			if(iSyst==6){
			...
			fit_lmean[iSyst]=lmean_[iSyst][cpm]*sqrt(12.);
			fit_X[iSyst]=0.;
			}
*/
			fit_errlmean[iSyst]=graph_->GetErrorYhigh(cpm);
			...
			double cpmcentreRealhigh=0;
				cout<<"bin is pT: "<<ptBin<<" and cpm: "<<cpm+1<<endl;
			for(int iSyst=0;iSyst<nSystematics;iSyst++){
//				lmean_Buffer=lmean_Buffer+TMath::Abs(lmean_[iSyst][cpm]);//ifMean
//				lmean_Buffer=lmean_Buffer+lmean_[iSyst][cpm];//ifChange
//				if(iSyst==6) lmean_[iSyst][cpm]=lmean_[iSyst][cpm]*sqrt(12.);
				cout<<"it is system number "<<iSyst<<" which has a value of "<<lmean_[iSyst][cpm]<<endl;
			...
			cout<<"the total lmean_buffer is "<<lmean_Buffer<<endl;
//			lmean[cpm]=lmean_Buffer/nSystematics;//ifMean
			lmean[cpm]=TMath::Sqrt(lmean_Buffer);//ifSquare
			cout<<"it's sqrt value is "<<lmean[cpm]<<endl;
...
/*
		      ...
		      cout<<"repeating again it is "<<lmean[cpm]<<endl;
...
//		TGraphAsymmErrors *graphSyst = new TGraphAsymmErrors(nBinscpm,cpmCentre_,lmean,cpmCentreErr_low,cpmCentreErr_high,0,0);//Original
		...
		graphSyst->SetMarkerColor(ToyMC::MarkerColor[ptBin]);
		graphSyst->SetLineColor(ToyMC::MarkerColor[ptBin]);
//		graphSyst->SetMarkerStyle(ToyMC::MarkerStyle[rapBin]);
		graphSyst->SetMarkerSize(2);
		graphSyst->SetName(GraphName);
