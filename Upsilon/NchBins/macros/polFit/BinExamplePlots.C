...
void plotSummary( TTree* Results, char parameter[200], char parameterTitle[200], int iState , int irapBin, int ipTbin, int cpmbin, char DataID[200] ,double nSigma) {
	...
	char savename[200];
	sprintf(savename,"FigBuffer/BinExamplePlots/%s/Ups%dS/%s_rap%d_pT%d_cpm%d.pdf",DataID,iState,parameter,irapBin,ipTbin,cpmbin);
	c2->SaveAs(savename);
...
void BinExamplePlots(){
	...
	gROOT->SetBatch();
	  ...
	  char DataID[200];
	  sprintf(DataID,"GreatCPMRun2_withsys");

	  bool PlotAcceptance=false;
	  ...
	  for(iState=nStatemin;iState<nStatemax+1;iState++){
		  ...
		  for(irapBin=rapmin;irapBin<rapmax+1;irapBin++){
			  for(ipTbin=pTmin;ipTbin<pTmax+1;ipTbin++){
			  	for(icpmbin=cpmmin;icpmbin<cpmmax+1;icpmbin++){


	  char Filename[200];
	  sprintf(Filename,"/data/users/ferraioc/Polarization/Modified/Data/%s/results_%dSUps_rap%d_pT%d_cpm%d.root",DataID,iState,irapBin,ipTbin,icpmbin);
      ...
      plotSummary( Results, parameter, parameterTitle, iState , irapBin, ipTbin,icpmbin, DataID , nSigma);
      ...
      h_lph_vs_lth_CS->SetMarkerColor(kGreen);
      h_lph_vs_lth_HX->SetMarkerColor(kRed);
      h_lph_vs_lth_PX->SetMarkerColor(kBlue);
...
//    	 plotLegend->SetTextFont(72);
         ...
         char savename[200];
         sprintf(savename,"FigBuffer/BinExamplePlots/%s/Ups%dS/lth_vs_lph_rap%d_pT%d_cpm%d.pdf",DataID,iState,irapBin,ipTbin,icpmbin);
         c1->SaveAs(savename);
...
sprintf(savename,"FigBuffer/BinExamplePlots/%s/Ups%dS/SubtractedBackground_rap%d_pT%d_cpm%d.pdf",DataID,iState,irapBin,ipTbin,icpmbin);
c2->SaveAs(savename);
...
sprintf(savename,"FigBuffer/BinExamplePlots/%s/Ups%dS/MetropolisHastingsAcceptanceCS_rap%d_pT%d_cpm%d.pdf",DataID,iState,irapBin,ipTbin,icpmbin);
c2->SaveAs(savename);
...
sprintf(savename,"FigBuffer/BinExamplePlots/%s/Ups%dS/MetropolisHastingsAcceptanceHX_rap%d_pT%d_cpm%d.pdf",DataID,iState,irapBin,ipTbin,icpmbin);
c2->SaveAs(savename);
...
sprintf(savename,"FigBuffer/BinExamplePlots/%s/Ups%dS/MetropolisHastingsAcceptancePX_rap%d_pT%d_cpm%d.pdf",DataID,iState,irapBin,ipTbin,icpmbin);
c2->SaveAs(savename);
}
resultFile->Close();
