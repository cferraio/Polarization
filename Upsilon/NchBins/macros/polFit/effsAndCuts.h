...
//  double pTBorderHLT[2][4] = {{4.6, 3.5, 2.75, 2.0}, {5.2, 4.0, 2.75, 2.0}};
  ...
  if(iCut==15){
	  if(pT>2.5) decision=kTRUE;
  }
  
  if(iCut==16){
	  if(TMath::Abs(eta)<1.4 && pT>3.5) decision=kTRUE;
	  if(TMath::Abs(eta)>1.4 && pT>3) decision=kTRUE;
  }
...
void EvaluateEffFileName(int nEff, char EffFileName [200], bool singleLeptonEff) {
	...
	if(singleLeptonEff) {
		...
		if(nEff==105) sprintf(EffFileName,"singleMuTruthEff_30April2013_40GeVrap1_startFromTracks_noKinemCuts_Dimuon0JpsiNoVertexing_RECOvariables.root");
		...
		if(nEff==1030) sprintf(EffFileName,"ParametrizedFactDataEff_Dec11_Central.root");//'soft'
		if(nEff==1031) sprintf(EffFileName,"ParametrizedFactDataEff_Dec11_pTshift_plus.root");
		if(nEff==1032) sprintf(EffFileName,"ParametrizedFactDataEff_Dec11_pTshift_minus.root");
		if(nEff==1033) sprintf(EffFileName,"ParametrizedFactDataEff_Dec11_pTscale_plus.root");
		if(nEff==1034) sprintf(EffFileName,"ParametrizedFactDataEff_Dec11_pTscale_minus.root");
		if(nEff==1035) sprintf(EffFileName,"ParametrizedFactDataEff_Dec11_effshift_plus.root");
		if(nEff==1036) sprintf(EffFileName,"ParametrizedFactDataEff_Dec11_effshift_minus.root");

		if(nEff==1040) sprintf(EffFileName,"ParametrizedFactDataEff_June12_Central.root");

		if(nEff==1050) sprintf(EffFileName,"ParametrizedFactDataEff_Dec11_Central.root");
		...
		if(nEff==1103) sprintf(EffFileName,"singleMuTruthEff_30April2013_40GeVrap1_startFromTracks_noKinemCuts_Dimuon0JpsiNoVertexing_RECOvariables.root");// gen pT
		
		if(nEff==1104) sprintf(EffFileName,"tnp_pPb_Prod_total_20130123.root");
...
//		if(nEff==331) sprintf(EffFileName,"results_1SUps_rap1_pT1_cpm2.root");
		...
		if(nEff==12104 || nEff==22104 || nEff==32104) sprintf(EffFileName,"MCTruthEfficiency_coarsePT_18July2012.root");
...
double EvaluateRhoFactor( double& costh, double& phi, int nEff, TFile* fInRhoFactor, double rap, double pT) {
	...
	if(nEff==331) {
	  char EffType[200];
	  sprintf(EffType,"signal_costhphiPX");
//sprintf(EffType,"histdiv2");
