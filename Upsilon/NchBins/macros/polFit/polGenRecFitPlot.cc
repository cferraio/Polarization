	...
	Char_t *TreeBinID_dataFile = "ToyMC"; //Storage Directory
	  ...
	  for( int i=0;i < argc; ++i ) {
		...
		if(injectRealDataPolarization){
			...
			if(frameSig==1)  {
				cout<<GraphName<<endl;
				sprintf(GraphName,"lth_CS_rap%d_pt%d",rapBin,ptBin);
				graph_lth = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"lph_CS_rap%d_pt%d",rapBin,ptBin);
				graph_lph = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"ltp_CS_rap%d_pt%d",rapBin,ptBin);
				graph_ltp = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"lthstar_CS_rap%d_pt%d",rapBin,ptBin);
				graph_lthstar = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"lphstar_CS_rap%d_pt%d",rapBin,ptBin);
				graph_lphstar = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"ltilde_CS_rap%d_pt%d",rapBin,ptBin);
				graph_ltilde = (TGraphAsymmErrors*) infile1->Get(GraphName);
	}

			if(frameSig==2)  {
				sprintf(GraphName,"lth_HX_rap%d_pt%d",rapBin,ptBin);
				graph_lth = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"lph_HX_rap%d_pt%d",rapBin,ptBin);
				graph_lph = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"ltp_HX_rap%d_pt%d",rapBin,ptBin);
				graph_ltp = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"lthstar_HX_rap%d_pt%d",rapBin,ptBin);
				graph_lthstar = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"lphstar_HX_rap%d_pt%d",rapBin,ptBin);
				graph_lphstar = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"ltilde_HX_rap%d_pt%d",rapBin,ptBin);
				graph_ltilde = (TGraphAsymmErrors*) infile1->Get(GraphName);
	}

			if(frameSig==3)  {
				sprintf(GraphName,"lth_PX_rap%d_pt%d",rapBin,ptBin);
				graph_lth = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"lph_PX_rap%d_pt%d",rapBin,ptBin);
				graph_lph = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"ltp_PX_rap%d_pt%d",rapBin,ptBin);
				graph_ltp = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"lthstar_PX_rap%d_pt%d",rapBin,ptBin);
				graph_lthstar = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"lphstar_PX_rap%d_pt%d",rapBin,ptBin);
				graph_lphstar = (TGraphAsymmErrors*) infile1->Get(GraphName);
				sprintf(GraphName,"ltilde_PX_rap%d_pt%d",rapBin,ptBin);
				graph_ltilde = (TGraphAsymmErrors*) infile1->Get(GraphName);
	}
...
/// Extract number of signal and background events to be generated, as well as f_BG to be generated to result in desired effective f_BG:
 	    ...
 	    int nTargetEvents;
 	    nTargetEvents = ToyMC::numEvents[iPt-1][iCPM-1];

 	    if(gen){

			int numEvCheck = 500000;
			f_BG = ToyMC::fracBackground[iPt-1][iCPM-1];
		...
		polGen(raplow,raphigh,ptlow,pthigh,cpmlow,cpmhigh,mass_signal_peak,mass_signal_sigma,n_sigmas_signal,numEvCheck,f_BG,lambda_theta_sig_,lambda_phi_sig_,lambda_thetaphi_sig_,lambda_theta_bkg_,lambda_phi_bkg_,lambda_thetaphi_bkg_,frameSig,frameBkg,-999,OutputDirectory);
  	    if(rec)polRec(raplow,raphigh,ptlow,pthigh,cpmlow,cpmhigh,mass_signal_peak,mass_signal_sigma,n_sigmas_signal,nRecEff,nRecDileptonEff,nRecRhoFactor,FidCuts,OutputDirectory, true, effDir, MCReceff, MCDileptonReceff, iRap, iPt,iCPM, useAmapApproach, nAmap, nDenominatorAmap);
			...
			cout << "1numEvCheck = " << numEvCheck << endl;
			cout << "1f_bg = " << f_BG << endl;
			cout << "1isBG_distribution = " << isBG_distribution->GetBinContent(1) << endl;
			double bkgFact = isBG_distribution->GetBinContent(2)/(numEvCheck*f_BG);
			cout << "2isBG_distribution = " << isBG_distribution->GetBinContent(2) << endl;
			dataFile->Close();


			if(ConstEvents_) nTargetEvents = ConstEvents;
			cout<< "ntargetevents = " << nTargetEvents << endl;
			cout << "nsigfact = " << sigFact << endl;
			cout << "f_bg = " << f_BG << endl;
			cout << "bkgfact = " << bkgFact << endl;
			n_events = nTargetEvents/sigFact+(nTargetEvents/(1-f_BG)-nTargetEvents)/bkgFact;
			cout << "nevents = " << n_events << endl;
			f_BG = (n_events-nTargetEvents/sigFact)/n_events;
			cout << "2f_bg = " << f_BG << endl;
