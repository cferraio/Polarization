...
void PlotRapPt(int cpmBinMin, int cpmBinMax, int ptBin, double yMin, double yMax, char yAxisTitle[200], char filename[200], double lmean[ToyMC::nCPMBins], double lmeanerr_low[ToyMC::nCPMBins], double lmeanerr_high[ToyMC::nCPMBins], bool DrawLine, double lamline, char GraphFolder[200], char GraphName[200], bool SaveGraph, int nState, int nFrame){
	...
	int ncpmBinsToPlot = cpmBinMax-cpmBinMin+1;
	double cpmCentre_[ncpmBinsToPlot];
	double cpmCentreErr_low[ncpmBinsToPlot];
	double cpmCentreErr_high[ncpmBinsToPlot];

	int cpm=0;
	for(int cpmbin = cpmBinMin; cpmbin < cpmBinMax+1; cpmbin ++) {
		cpmCentre_[cpm]=ToyMC::cpmCentre[ptBin-1][cpmbin-1];
		cpmCentreErr_low[cpm]=TMath::Abs(ToyMC::cpmCentre[ptBin-1][cpmbin-1]-onia::cpmRange[cpmbin-1]);
		cpmCentreErr_high[cpm]=TMath::Abs(ToyMC::cpmCentre[ptBin-1][cpmbin-1]-onia::cpmRange[cpmbin]);
	cpm ++;
	}

 	TGraphAsymmErrors *plotGraph = new TGraphAsymmErrors(ncpmBinsToPlot, cpmCentre_,lmean, cpmCentreErr_low, cpmCentreErr_high,lmeanerr_low,lmeanerr_high);

	plotGraph->SetMarkerColor(ToyMC::MarkerColor[nFrame]);
	plotGraph->SetLineColor(ToyMC::MarkerColor[nFrame]);
	plotGraph->SetMarkerStyle(ToyMC::MarkerStyle[nState][ptBin]);
	plotGraph->SetMarkerSize(ToyMC::MarkerSize[nState][ptBin]);
	...
	char texTex[200];
	if(ptBin==1) sprintf(texTex,"10 - 15 GeV");
	if(ptBin==2) sprintf(texTex,"15 - 50 GeV");
...
void PlotComparisonRapPt(int cpmBinMin,int cpmBinMax,int ptBin, double yMin, double yMax, char yAxisTitle[200], char filename[200], double lmean1[ToyMC::nCPMBins], double lmean1err_low[ToyMC::nCPMBins], double lmean1err_high[ToyMC::nCPMBins], double lmean2[ToyMC::nCPMBins], double lmean2err_low[ToyMC::nCPMBins], double lmean2err_high[ToyMC::nCPMBins], double lmean3[ToyMC::nCPMBins], double lmean3err_low[ToyMC::nCPMBins], double lmean3err_high[ToyMC::nCPMBins], double lamline, int nState){
	...
	int cpm=0;
	for(int cpmBin = cpmBinMin; cpmBin < cpmBinMax+1; cpmBin++) {
		cpmCentre_[cpm]=ToyMC::cpmCentre[ptBin-1][cpmBin-1]+ToyMC::cpmCentre[ptBin-1][cpmBin-1]*.02;
		cpmCentreErr_low[cpm]=TMath::Abs(ToyMC::cpmCentre[ptBin-1][cpmBin-1]-onia::cpmRange[cpmBin-1]);
		cpmCentreErr_high[cpm]=TMath::Abs(ToyMC::cpmCentre[ptBin-1][cpmBin-1]-onia::cpmRange[cpmBin]);
	cpm++;
	}
	...
	cpm=0;
	for(int cpmBin = cpmBinMin; cpmBin < cpmBinMax+1; cpmBin++) {
		cpmCentre_[cpm]=ToyMC::cpmCentre[ptBin-1][cpmBin-1]-ToyMC::cpmCentre[cpmBin-1][cpmBin-1]*.02;
		cpmCentreErr_low[cpm]=TMath::Abs(ToyMC::cpmCentre[cpmBin-1][cpmBin-1]-onia::cpmRange[cpmBin-1]);
		cpmCentreErr_high[cpm]=TMath::Abs(ToyMC::cpmCentre[cpmBin-1][cpmBin-1]-onia::cpmRange[cpmBin]);
	cpm++;
	}
	...
	char texTex[200];
	if(ptBin==1) sprintf(texTex," 10 <  p_{T} < 15");
	if(ptBin==2) sprintf(texTex,"15 < p_{T} < 50");
	 TLatex *text = new TLatex(onia::cpmRange[cpmBinMax]*0.22,yMin+(yMax-yMin)*0.1,texTex);
	 text->SetTextSize(0.035);
	 text->Draw( "same" );
...
///// Extract numerical values of the lambda parameters from the result TTrees /////
	  ...
	  if(nGen_[rap-1][pt-1][cpm-1]==0) {emptyBin[rap-1][pt-1][cpm-1]=true; cout<<"empty bin"<<endl;}
	    ...
	    /*	    sigma1Counter_lth_CS[rap-1][pt-1][cpm-1]=0;
	    	    for(int iPull=0;iPull<int((l_max_pull-l_min_pull)/l_step_1D_pull)+1;iPull++){
	    	    	if(TMath::Abs(param_lth_CS->GetBinCenter())<1) sigma1Counter_lth_CS[rap-1][pt-1][cpm-1]+=param_lth_CS->GetBinContent();
	    	    }
	    	    sigma1Counter_lth_CS[rap-1][pt-1][cpm-1]=sigma1Counter_lth_CS[rap-1][pt-1][cpm-1]/param_lth_CS->GetSumOfWeights();
	    	    pull_lth_CS_mean[rap-1][pt-1][cpm-1]=pull_lth_CS->GetMean(); pull_lth_CS_meanerr[rap-1][pt-1][cpm-1]=pull_lth_CS->GetMeanError();
	    	    pull_lth_CS_sigma[rap-1][pt-1][cpm-1]=pull_lth_CS->GetRMS();
	    	    pull_lth_CS_sigmaerr[rap-1][pt-1][cpm-1]=pull_lth_CS->GetRMSError();
...
//////////////////////////////////////////////////
	    ...
	    double KinDepRap[NcpmBins];
	    double KinDepRaperr_low[NcpmBins];
	    double KinDepRaperr_high[NcpmBins];
	    double KinDepRap2[NcpmBins];
	    double KinDepRap2err_low[NcpmBins];
	    double KinDepRap2err_high[NcpmBins];
	    double KinDepRap3[NcpmBins];
	    double KinDepRap3err_low[NcpmBins];
	    double KinDepRap3err_high[NcpmBins];
	    ...
	    for(int ptBin = ptBinMin; ptBin < ptBinMax+1; ptBin++) {
	    	...
	    	if(iLam==105){lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_mean_Comparison_lphstar_rap%d_pt%d.pdf",dirstruct,rapBin,ptBin); 	sprintf(axislabel,"#lambda*_{#phi}"); lamline=lambda_phistar_injected_CS[rapBin-1][ptBin-1][0];}
					  ...
					  for(int cpmBin = cpmBinMin; cpmBin < cpmBinMax+1; cpmBin++) {
						...
						if(iLam==13){ KinDepRap[cpm]= param_lth_CS_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_lth_CS_meanerr[rap][pt][cpm];		 	if(RealData) {KinDepRaperr_low[cpm]= param_lth_CS_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_lth_CS_sigma_high[rap][pt][cpm];}    }
						if(iLam==14){ KinDepRap[cpm]= param_lth_CS_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lth_CS_sigmaerr[rap][pt][cpm];		}
						if(iLam==15){ KinDepRap[cpm]= param_lph_CS_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_lph_CS_meanerr[rap][pt][cpm]; 		if(RealData) {KinDepRaperr_low[cpm]= param_lph_CS_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_lph_CS_sigma_high[rap][pt][cpm];}    }
						if(iLam==16){ KinDepRap[cpm]= param_lph_CS_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lph_CS_sigmaerr[rap][pt][cpm];		}
						if(iLam==17){ KinDepRap[cpm]= param_ltp_CS_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_ltp_CS_meanerr[rap][pt][cpm]; 		if(RealData) {KinDepRaperr_low[cpm]= param_ltp_CS_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_ltp_CS_sigma_high[rap][pt][cpm];}    }
						if(iLam==18){ KinDepRap[cpm]= param_ltp_CS_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_ltp_CS_sigmaerr[rap][pt][cpm];		}
						if(iLam==19){ KinDepRap[cpm]= param_lthstar_CS_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lthstar_CS_meanerr[rap][pt][cpm]; 	if(RealData) {KinDepRaperr_low[cpm]= param_lthstar_CS_sigma_low[rap][pt][cpm];	KinDepRaperr_high[cpm]= param_lthstar_CS_sigma_high[rap][pt][cpm];}}
						if(iLam==20){ KinDepRap[cpm]= param_lthstar_CS_sigma[rap][pt][cpm]; KinDepRaperr_low[cpm]=param_lthstar_CS_sigmaerr[rap][pt][cpm];	}
						if(iLam==21){ KinDepRap[cpm]= param_lphstar_CS_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lphstar_CS_meanerr[rap][pt][cpm]; 	if(RealData) {KinDepRaperr_low[cpm]= param_lphstar_CS_sigma_low[rap][pt][cpm];	KinDepRaperr_high[cpm]= param_lphstar_CS_sigma_high[rap][pt][cpm];}}
						if(iLam==22){ KinDepRap[cpm]= param_lphstar_CS_sigma[rap][pt][cpm]; KinDepRaperr_low[cpm]=param_lphstar_CS_sigmaerr[rap][pt][cpm];	}
						if(iLam==23){ KinDepRap[cpm]= param_ltilde_CS_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_ltilde_CS_meanerr[rap][pt][cpm]; 		if(RealData) {KinDepRaperr_low[cpm]= param_ltilde_CS_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_ltilde_CS_sigma_high[rap][pt][cpm];} }
						if(iLam==24){ KinDepRap[cpm]= param_ltilde_CS_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_ltilde_CS_sigmaerr[rap][pt][cpm];		}

						if(iLam==25){ KinDepRap[cpm]= param_lth_HX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_lth_HX_meanerr[rap][pt][cpm]; 		if(RealData) {KinDepRaperr_low[cpm]= param_lth_HX_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_lth_HX_sigma_high[rap][pt][cpm];}    }
						if(iLam==26){ KinDepRap[cpm]= param_lth_HX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lth_HX_sigmaerr[rap][pt][cpm];		}
						if(iLam==27){ KinDepRap[cpm]= param_lph_HX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_lph_HX_meanerr[rap][pt][cpm]; 		if(RealData) {KinDepRaperr_low[cpm]= param_lph_HX_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_lph_HX_sigma_high[rap][pt][cpm];}    }
						if(iLam==28){ KinDepRap[cpm]= param_lph_HX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lph_HX_sigmaerr[rap][pt][cpm];		}
						if(iLam==29){ KinDepRap[cpm]= param_ltp_HX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_ltp_HX_meanerr[rap][pt][cpm]; 		if(RealData) {KinDepRaperr_low[cpm]= param_ltp_HX_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_ltp_HX_sigma_high[rap][pt][cpm];}    }
						if(iLam==30){ KinDepRap[cpm]= param_ltp_HX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_ltp_HX_sigmaerr[rap][pt][cpm];		}
						if(iLam==31){ KinDepRap[cpm]= param_lthstar_HX_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lthstar_HX_meanerr[rap][pt][cpm]; 	if(RealData) {KinDepRaperr_low[cpm]= param_lthstar_HX_sigma_low[rap][pt][cpm];	KinDepRaperr_high[cpm]= param_lthstar_HX_sigma_high[rap][pt][cpm];}}
						if(iLam==32){ KinDepRap[cpm]= param_lthstar_HX_sigma[rap][pt][cpm]; KinDepRaperr_low[cpm]=param_lthstar_HX_sigmaerr[rap][pt][cpm];	}
						if(iLam==33){ KinDepRap[cpm]= param_lphstar_HX_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lphstar_HX_meanerr[rap][pt][cpm]; 	if(RealData) {KinDepRaperr_low[cpm]= param_lphstar_HX_sigma_low[rap][pt][cpm];	KinDepRaperr_high[cpm]= param_lphstar_HX_sigma_high[rap][pt][cpm];}}
						if(iLam==34){ KinDepRap[cpm]= param_lphstar_HX_sigma[rap][pt][cpm]; KinDepRaperr_low[cpm]=param_lphstar_HX_sigmaerr[rap][pt][cpm];	}
						if(iLam==35){ KinDepRap[cpm]= param_ltilde_HX_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_ltilde_HX_meanerr[rap][pt][cpm]; 		if(RealData) {KinDepRaperr_low[cpm]= param_ltilde_HX_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_ltilde_HX_sigma_high[rap][pt][cpm];} }
						if(iLam==36){ KinDepRap[cpm]= param_ltilde_HX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_ltilde_HX_sigmaerr[rap][pt][cpm];		}

						if(iLam==37){ KinDepRap[cpm]= param_lth_PX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_lth_PX_meanerr[rap][pt][cpm]; 		if(RealData) {KinDepRaperr_low[cpm]= param_lth_PX_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_lth_PX_sigma_high[rap][pt][cpm];}    }
						if(iLam==38){ KinDepRap[cpm]= param_lth_PX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lth_PX_sigmaerr[rap][pt][cpm];		}
						if(iLam==39){ KinDepRap[cpm]= param_lph_PX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_lph_PX_meanerr[rap][pt][cpm]; 		if(RealData) {KinDepRaperr_low[cpm]= param_lph_PX_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_lph_PX_sigma_high[rap][pt][cpm];}    }
						if(iLam==40){ KinDepRap[cpm]= param_lph_PX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lph_PX_sigmaerr[rap][pt][cpm];		}
						if(iLam==41){ KinDepRap[cpm]= param_ltp_PX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_ltp_PX_meanerr[rap][pt][cpm]; 		if(RealData) {KinDepRaperr_low[cpm]= param_ltp_PX_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_ltp_PX_sigma_high[rap][pt][cpm];}    }
						if(iLam==42){ KinDepRap[cpm]= param_ltp_PX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_ltp_PX_sigmaerr[rap][pt][cpm];		}
						if(iLam==43){ KinDepRap[cpm]= param_lthstar_PX_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lthstar_PX_meanerr[rap][pt][cpm];	 	if(RealData) {KinDepRaperr_low[cpm]= param_lthstar_PX_sigma_low[rap][pt][cpm];	KinDepRaperr_high[cpm]= param_lthstar_PX_sigma_high[rap][pt][cpm];}}
						if(iLam==44){ KinDepRap[cpm]= param_lthstar_PX_sigma[rap][pt][cpm]; KinDepRaperr_low[cpm]=param_lthstar_PX_sigmaerr[rap][pt][cpm];	}
						if(iLam==45){ KinDepRap[cpm]= param_lphstar_PX_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lphstar_PX_meanerr[rap][pt][cpm]; 	if(RealData) {KinDepRaperr_low[cpm]= param_lphstar_PX_sigma_low[rap][pt][cpm];	KinDepRaperr_high[cpm]= param_lphstar_PX_sigma_high[rap][pt][cpm];}}
						if(iLam==46){ KinDepRap[cpm]= param_lphstar_PX_sigma[rap][pt][cpm]; KinDepRaperr_low[cpm]=param_lphstar_PX_sigmaerr[rap][pt][cpm];	}
						if(iLam==47){ KinDepRap[cpm]= param_ltilde_PX_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_ltilde_PX_meanerr[rap][pt][cpm]; 		if(RealData) {KinDepRaperr_low[cpm]= param_ltilde_PX_sigma_low[rap][pt][cpm];		KinDepRaperr_high[cpm]= param_ltilde_PX_sigma_high[rap][pt][cpm];} }
						if(iLam==48){ KinDepRap[cpm]= param_ltilde_PX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_ltilde_PX_sigmaerr[rap][pt][cpm];		}

						if(iLam==49){ KinDepRap[cpm]= pull_lth_CS_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lth_CS_meanerr[rap][pt][cpm];			}
						if(iLam==50){ KinDepRap[cpm]= pull_lth_CS_sigma[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lth_CS_sigmaerr[rap][pt][cpm];			}
						if(iLam==51){ KinDepRap[cpm]= pull_lph_CS_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lph_CS_meanerr[rap][pt][cpm];			}
						if(iLam==52){ KinDepRap[cpm]= pull_lph_CS_sigma[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lph_CS_sigmaerr[rap][pt][cpm];			}
						if(iLam==53){ KinDepRap[cpm]= pull_ltp_CS_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_ltp_CS_meanerr[rap][pt][cpm];			}
						if(iLam==54){ KinDepRap[cpm]= pull_ltp_CS_sigma[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_ltp_CS_sigmaerr[rap][pt][cpm];			}
						if(iLam==55){ KinDepRap[cpm]= pull_lthstar_CS_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lthstar_CS_meanerr[rap][pt][cpm];		}
						if(iLam==56){ KinDepRap[cpm]= pull_lthstar_CS_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lthstar_CS_sigmaerr[rap][pt][cpm];		}
						if(iLam==57){ KinDepRap[cpm]= pull_lphstar_CS_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lphstar_CS_meanerr[rap][pt][cpm];		}
						if(iLam==58){ KinDepRap[cpm]= pull_lphstar_CS_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lphstar_CS_sigmaerr[rap][pt][cpm];		}
						if(iLam==59){ KinDepRap[cpm]= pull_ltilde_CS_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_ltilde_CS_meanerr[rap][pt][cpm];		}
						if(iLam==60){ KinDepRap[cpm]= pull_ltilde_CS_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_ltilde_CS_sigmaerr[rap][pt][cpm];		}

						if(iLam==61){ KinDepRap[cpm]= pull_lth_HX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lth_HX_meanerr[rap][pt][cpm];			}
						if(iLam==62){ KinDepRap[cpm]= pull_lth_HX_sigma[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lth_HX_sigmaerr[rap][pt][cpm];			}
						if(iLam==63){ KinDepRap[cpm]= pull_lph_HX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lph_HX_meanerr[rap][pt][cpm];			}
						if(iLam==64){ KinDepRap[cpm]= pull_lph_HX_sigma[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lph_HX_sigmaerr[rap][pt][cpm];			}
						if(iLam==65){ KinDepRap[cpm]= pull_ltp_HX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_ltp_HX_meanerr[rap][pt][cpm];			}
						if(iLam==66){ KinDepRap[cpm]= pull_ltp_HX_sigma[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_ltp_HX_sigmaerr[rap][pt][cpm];			}
						if(iLam==67){ KinDepRap[cpm]= pull_lthstar_HX_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lthstar_HX_meanerr[rap][pt][cpm];		}
						if(iLam==68){ KinDepRap[cpm]= pull_lthstar_HX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lthstar_HX_sigmaerr[rap][pt][cpm];		}
						if(iLam==69){ KinDepRap[cpm]= pull_lphstar_HX_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lphstar_HX_meanerr[rap][pt][cpm];		}
						if(iLam==70){ KinDepRap[cpm]= pull_lphstar_HX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lphstar_HX_sigmaerr[rap][pt][cpm];		}
						if(iLam==71){ KinDepRap[cpm]= pull_ltilde_HX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_ltilde_HX_meanerr[rap][pt][cpm];		}
						if(iLam==72){ KinDepRap[cpm]= pull_ltilde_HX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_ltilde_HX_sigmaerr[rap][pt][cpm];		}

						if(iLam==73){ KinDepRap[cpm]= pull_lth_PX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lth_PX_meanerr[rap][pt][cpm];			}
						if(iLam==74){ KinDepRap[cpm]= pull_lth_PX_sigma[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lth_PX_sigmaerr[rap][pt][cpm];			}
						if(iLam==75){ KinDepRap[cpm]= pull_lph_PX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lph_PX_meanerr[rap][pt][cpm];			}
						if(iLam==76){ KinDepRap[cpm]= pull_lph_PX_sigma[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_lph_PX_sigmaerr[rap][pt][cpm];			}
						if(iLam==77){ KinDepRap[cpm]= pull_ltp_PX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_ltp_PX_meanerr[rap][pt][cpm];			}
						if(iLam==78){ KinDepRap[cpm]= pull_ltp_PX_sigma[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_ltp_PX_sigmaerr[rap][pt][cpm];			}
						if(iLam==79){ KinDepRap[cpm]= pull_lthstar_PX_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lthstar_PX_meanerr[rap][pt][cpm];		}
						if(iLam==80){ KinDepRap[cpm]= pull_lthstar_PX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lthstar_PX_sigmaerr[rap][pt][cpm];		}
						if(iLam==81){ KinDepRap[cpm]= pull_lphstar_PX_mean[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lphstar_PX_meanerr[rap][pt][cpm];		}
						if(iLam==82){ KinDepRap[cpm]= pull_lphstar_PX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_lphstar_PX_sigmaerr[rap][pt][cpm];		}
						if(iLam==83){ KinDepRap[cpm]= pull_ltilde_PX_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=pull_ltilde_PX_meanerr[rap][pt][cpm];		}
						if(iLam==84){ KinDepRap[cpm]= pull_ltilde_PX_sigma[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=pull_ltilde_PX_sigmaerr[rap][pt][cpm];		}


						if(iLam==85){ KinDepRap[cpm]= param_lth_CS_mean[rap][pt][cpm]-lambda_theta_injected_CS[rap][pt][cpm]; 			KinDepRaperr_low[cpm]=param_lth_CS_sigma[rap][pt][cpm];}
						if(iLam==86){ KinDepRap[cpm]= param_lph_CS_mean[rap][pt][cpm]-lambda_phi_injected_CS[rap][pt][cpm]; 			KinDepRaperr_low[cpm]=param_lph_CS_sigma[rap][pt][cpm];}
						if(iLam==87){ KinDepRap[cpm]= param_ltp_CS_mean[rap][pt][cpm]-lambda_thetaphi_injected_CS[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_ltp_CS_sigma[rap][pt][cpm];}
						if(iLam==88){ KinDepRap[cpm]= param_lthstar_CS_mean[rap][pt][cpm]-lambda_thetastar_injected_CS[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lthstar_CS_sigma[rap][pt][cpm];}
						if(iLam==89){ KinDepRap[cpm]= param_lphstar_CS_mean[rap][pt][cpm]-lambda_phistar_injected_CS[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lphstar_CS_sigma[rap][pt][cpm];}
						if(iLam==90){ KinDepRap[cpm]= param_ltilde_CS_mean[rap][pt][cpm]-lambda_tilde_injected_CS[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_ltilde_CS_sigma[rap][pt][cpm];}

						if(iLam==91){ KinDepRap[cpm]= param_lth_HX_mean[rap][pt][cpm]-lambda_theta_injected_HX[rap][pt][cpm]; 			KinDepRaperr_low[cpm]=param_lth_HX_sigma[rap][pt][cpm];}
						if(iLam==92){ KinDepRap[cpm]= param_lph_HX_mean[rap][pt][cpm]-lambda_phi_injected_HX[rap][pt][cpm]; 			KinDepRaperr_low[cpm]=param_lph_HX_sigma[rap][pt][cpm];}
						if(iLam==93){ KinDepRap[cpm]= param_ltp_HX_mean[rap][pt][cpm]-lambda_thetaphi_injected_HX[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_ltp_HX_sigma[rap][pt][cpm];}
						if(iLam==94){ KinDepRap[cpm]= param_lthstar_HX_mean[rap][pt][cpm]-lambda_thetastar_injected_HX[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lthstar_HX_sigma[rap][pt][cpm];}
						if(iLam==95){ KinDepRap[cpm]= param_lphstar_HX_mean[rap][pt][cpm]-lambda_phistar_injected_HX[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lphstar_HX_sigma[rap][pt][cpm];}
						if(iLam==96){ KinDepRap[cpm]= param_ltilde_HX_mean[rap][pt][cpm]-lambda_tilde_injected_HX[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_ltilde_HX_sigma[rap][pt][cpm];}

						if(iLam==97){ KinDepRap[cpm]= param_lth_PX_mean[rap][pt][cpm]-lambda_theta_injected_PX[rap][pt][cpm]; 			KinDepRaperr_low[cpm]=param_lth_PX_sigma[rap][pt][cpm];}
						if(iLam==98){ KinDepRap[cpm]= param_lph_PX_mean[rap][pt][cpm]-lambda_phi_injected_PX[rap][pt][cpm]; 			KinDepRaperr_low[cpm]=param_lph_PX_sigma[rap][pt][cpm];}
						if(iLam==99){ KinDepRap[cpm]= param_ltp_PX_mean[rap][pt][cpm]-lambda_thetaphi_injected_PX[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_ltp_PX_sigma[rap][pt][cpm];}
						if(iLam==100){ KinDepRap[cpm]= param_lthstar_PX_mean[rap][pt][cpm]-lambda_thetastar_injected_PX[rap][pt][cpm]; KinDepRaperr_low[cpm]=param_lthstar_PX_sigma[rap][pt][cpm];}
						if(iLam==101){ KinDepRap[cpm]= param_lphstar_PX_mean[rap][pt][cpm]-lambda_phistar_injected_PX[rap][pt][cpm]; 	KinDepRaperr_low[cpm]=param_lphstar_PX_sigma[rap][pt][cpm];}
						if(iLam==102){ KinDepRap[cpm]= param_ltilde_PX_mean[rap][pt][cpm]-lambda_tilde_injected_PX[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_ltilde_PX_sigma[rap][pt][cpm];}

						if(iLam==103){ KinDepRap[cpm]= param_ltilde_CS_mean[rap][pt][cpm]; 		    KinDepRaperr_low[cpm]=param_ltilde_CS_sigma_low[rap][pt][cpm];	KinDepRaperr_high[cpm]=param_ltilde_CS_sigma_high[rap][pt][cpm];	KinDepRap2[cpm]= param_ltilde_HX_mean[rap][pt][cpm]; 		KinDepRap2err_low[cpm]=param_ltilde_HX_sigma_low[rap][pt][cpm];	    KinDepRap2err_high[cpm]=param_ltilde_HX_sigma_high[rap][pt][cpm];	KinDepRap2[cpm]= param_ltilde_PX_mean[rap][pt][cpm]; 		KinDepRap3err_low[cpm]=param_ltilde_PX_sigma_low[rap][pt][cpm];  	KinDepRap3err_high[cpm]=param_ltilde_PX_sigma_high[rap][pt][cpm];}
						if(iLam==104){ KinDepRap[cpm]= param_lthstar_CS_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_lthstar_CS_sigma_low[rap][pt][cpm];	KinDepRaperr_high[cpm]=param_lthstar_CS_sigma_high[rap][pt][cpm];	KinDepRap2[cpm]= param_lthstar_HX_mean[rap][pt][cpm]; 		KinDepRap2err_low[cpm]=param_lthstar_HX_sigma_low[rap][pt][cpm];	KinDepRap2err_high[cpm]=param_lthstar_HX_sigma_high[rap][pt][cpm];	KinDepRap2[cpm]= param_lthstar_PX_mean[rap][pt][cpm]; 		KinDepRap3err_low[cpm]=param_lthstar_PX_sigma_low[rap][pt][cpm];	KinDepRap3err_high[cpm]=param_lthstar_PX_sigma_high[rap][pt][cpm];}
						if(iLam==105){ KinDepRap[cpm]= param_lphstar_CS_mean[rap][pt][cpm]; 		KinDepRaperr_low[cpm]=param_lphstar_CS_sigma_low[rap][pt][cpm];	KinDepRaperr_high[cpm]=param_lphstar_CS_sigma_high[rap][pt][cpm];	KinDepRap2[cpm]= param_lphstar_HX_mean[rap][pt][cpm]; 		KinDepRap2err_low[cpm]=param_lphstar_HX_sigma_low[rap][pt][cpm];	KinDepRap2err_high[cpm]=param_lphstar_HX_sigma_high[rap][pt][cpm];	KinDepRap2[cpm]= param_lphstar_PX_mean[rap][pt][cpm]; 		KinDepRap3err_low[cpm]=param_lphstar_PX_sigma_low[rap][pt][cpm];	KinDepRap3err_high[cpm]=param_lphstar_PX_sigma_high[rap][pt][cpm];}

						if(emptyBin[rap][pt][cpm]){ KinDepRap[cpm]=999; KinDepRap2[cpm]=999; KinDepRap2[cpm]=999;}

						if(iLam<103 && !RealData) {KinDepRaperr_high[cpm]=KinDepRaperr_low[cpm];}
					...
					if(iLam<103) PlotRapPt(cpmBinMin,cpmBinMax,ptBin, lammin, lammax, axislabel, filename, KinDepRap, KinDepRaperr_low, KinDepRaperr_high, DrawLine, lamline, GraphFolder, GraphName, SaveGraph, nState, nFrame);
					else PlotComparisonRapPt(cpmBinMin,cpmBinMax,ptBin, lammin, lammax, axislabel, filename, KinDepRap, KinDepRaperr_low, KinDepRaperr_high, KinDepRap2, KinDepRap2err_low, KinDepRap2err_high, KinDepRap3, KinDepRap3err_low, KinDepRap3err_high, lamline, nState);
					pt++;
					}
					rap++;
	    }
				...
				int pt=0;
					for(int ptBin = ptBinMin; ptBin < ptBinMax+1; ptBin++) {
						...
						if (!emptyBin[rap][pt][cpm]) {
							meandeviation[0][0][0]=meandeviation[0][0][0]+TMath::Abs((param_lth_CS_mean[rap][pt][cpm]-lambda_theta_injected_CS[rap][pt][cpm]))/param_lth_CS_sigma[rap][pt][cpm];
							meandeviation[0][0][1]=meandeviation[0][0][1]+TMath::Abs((param_lph_CS_mean[rap][pt][cpm]-lambda_phi_injected_CS[rap][pt][cpm]))/param_lph_CS_sigma[rap][pt][cpm];
							meandeviation[0][0][2]=meandeviation[0][0][2]+TMath::Abs((param_ltp_CS_mean[rap][pt][cpm]-lambda_thetaphi_injected_CS[rap][pt][cpm]))/param_ltp_CS_sigma[rap][pt][cpm];
							meandeviation[0][0][3]=meandeviation[0][0][3]+TMath::Abs((param_ltilde_CS_mean[rap][pt][cpm]-lambda_tilde_injected_CS[rap][pt][cpm]))/param_ltilde_CS_sigma[rap][pt][cpm];
							meandeviation[0][0][4]=meandeviation[0][0][4]+TMath::Abs((param_lthstar_CS_mean[rap][pt][cpm]-lambda_thetastar_injected_CS[rap][pt][cpm]))/param_lthstar_CS_sigma[rap][pt][cpm];
							meandeviation[0][0][5]=meandeviation[0][0][5]+TMath::Abs((param_lphstar_CS_mean[rap][pt][cpm]-lambda_phistar_injected_CS[rap][pt][cpm]))/param_lphstar_CS_sigma[rap][pt][cpm];

							meandeviation[0][1][0]=meandeviation[0][1][0]+TMath::Abs((param_lth_HX_mean[rap][pt][cpm]-lambda_theta_injected_HX[rap][pt][cpm]))/param_lth_HX_sigma[rap][pt][cpm];
							meandeviation[0][1][1]=meandeviation[0][1][1]+TMath::Abs((param_lph_HX_mean[rap][pt][cpm]-lambda_phi_injected_HX[rap][pt][cpm]))/param_lph_HX_sigma[rap][pt][cpm];
							meandeviation[0][1][2]=meandeviation[0][1][2]+TMath::Abs((param_ltp_HX_mean[rap][pt][cpm]-lambda_thetaphi_injected_HX[rap][pt][cpm]))/param_ltp_HX_sigma[rap][pt][cpm];
							meandeviation[0][1][3]=meandeviation[0][1][3]+TMath::Abs((param_ltilde_HX_mean[rap][pt][cpm]-lambda_tilde_injected_HX[rap][pt][cpm]))/param_ltilde_HX_sigma[rap][pt][cpm];
							meandeviation[0][1][4]=meandeviation[0][1][4]+TMath::Abs((param_lthstar_HX_mean[rap][pt][cpm]-lambda_thetastar_injected_HX[rap][pt][cpm]))/param_lthstar_HX_sigma[rap][pt][cpm];
							meandeviation[0][1][5]=meandeviation[0][1][5]+TMath::Abs((param_lphstar_HX_mean[rap][pt][cpm]-lambda_phistar_injected_HX[rap][pt][cpm]))/param_lphstar_HX_sigma[rap][pt][cpm];

							meandeviation[0][2][0]=meandeviation[0][2][0]+TMath::Abs((param_lth_PX_mean[rap][pt][cpm]-lambda_theta_injected_PX[rap][pt][cpm]))/param_lth_PX_sigma[rap][pt][cpm];
							meandeviation[0][2][1]=meandeviation[0][2][1]+TMath::Abs((param_lph_PX_mean[rap][pt][cpm]-lambda_phi_injected_PX[rap][pt][cpm]))/param_lph_PX_sigma[rap][pt][cpm];
							meandeviation[0][2][2]=meandeviation[0][2][2]+TMath::Abs((param_ltp_PX_mean[rap][pt][cpm]-lambda_thetaphi_injected_PX[rap][pt][cpm]))/param_ltp_PX_sigma[rap][pt][cpm];
							meandeviation[0][2][3]=meandeviation[0][2][3]+TMath::Abs((param_ltilde_PX_mean[rap][pt][cpm]-lambda_tilde_injected_PX[rap][pt][cpm]))/param_ltilde_PX_sigma[rap][pt][cpm];
							meandeviation[0][2][4]=meandeviation[0][2][4]+TMath::Abs((param_lthstar_PX_mean[rap][pt][cpm]-lambda_thetastar_injected_PX[rap][pt][cpm]))/param_lthstar_PX_sigma[rap][pt][cpm];
							meandeviation[0][2][5]=meandeviation[0][2][5]+TMath::Abs((param_lphstar_PX_mean[rap][pt][cpm]-lambda_phistar_injected_PX[rap][pt][cpm]))/param_lphstar_PX_sigma[rap][pt][cpm];


							meandeviation[1][0][0]=meandeviation[1][0][0]+TMath::Abs(pull_lth_CS_mean[rap][pt][cpm])/pull_lth_CS_meanerr[rap][pt][cpm];
							meandeviation[1][0][1]=meandeviation[1][0][1]+TMath::Abs(pull_lph_CS_mean[rap][pt][cpm])/pull_lph_CS_meanerr[rap][pt][cpm];
							meandeviation[1][0][2]=meandeviation[1][0][2]+TMath::Abs(pull_ltp_CS_mean[rap][pt][cpm])/pull_ltp_CS_meanerr[rap][pt][cpm];
							meandeviation[1][0][3]=meandeviation[1][0][3]+TMath::Abs(pull_ltilde_CS_mean[rap][pt][cpm])/pull_ltilde_CS_meanerr[rap][pt][cpm];
							meandeviation[1][0][4]=meandeviation[1][0][4]+TMath::Abs(pull_lthstar_CS_mean[rap][pt][cpm])/pull_lthstar_CS_meanerr[rap][pt][cpm];
							meandeviation[1][0][5]=meandeviation[1][0][5]+TMath::Abs(pull_lphstar_CS_mean[rap][pt][cpm])/pull_lphstar_CS_meanerr[rap][pt][cpm];

							meandeviation[1][1][0]=meandeviation[1][1][0]+TMath::Abs(pull_lth_HX_mean[rap][pt][cpm])/pull_lth_HX_meanerr[rap][pt][cpm];
							meandeviation[1][1][1]=meandeviation[1][1][1]+TMath::Abs(pull_lph_HX_mean[rap][pt][cpm])/pull_lph_HX_meanerr[rap][pt][cpm];
							meandeviation[1][1][2]=meandeviation[1][1][2]+TMath::Abs(pull_ltp_HX_mean[rap][pt][cpm])/pull_ltp_HX_meanerr[rap][pt][cpm];
							meandeviation[1][1][3]=meandeviation[1][1][3]+TMath::Abs(pull_ltilde_HX_mean[rap][pt][cpm])/pull_ltilde_HX_meanerr[rap][pt][cpm];
							meandeviation[1][1][4]=meandeviation[1][1][4]+TMath::Abs(pull_lthstar_HX_mean[rap][pt][cpm])/pull_lthstar_HX_meanerr[rap][pt][cpm];
							meandeviation[1][1][5]=meandeviation[1][1][5]+TMath::Abs(pull_lphstar_HX_mean[rap][pt][cpm])/pull_lphstar_HX_meanerr[rap][pt][cpm];

							meandeviation[1][2][0]=meandeviation[1][2][0]+TMath::Abs(pull_lth_PX_mean[rap][pt][cpm])/pull_lth_PX_meanerr[rap][pt][cpm];
							meandeviation[1][2][1]=meandeviation[1][2][1]+TMath::Abs(pull_lph_PX_mean[rap][pt][cpm])/pull_lph_PX_meanerr[rap][pt][cpm];
							meandeviation[1][2][2]=meandeviation[1][2][2]+TMath::Abs(pull_ltp_PX_mean[rap][pt][cpm])/pull_ltp_PX_meanerr[rap][pt][cpm];
							meandeviation[1][2][3]=meandeviation[1][2][3]+TMath::Abs(pull_ltilde_PX_mean[rap][pt][cpm])/pull_ltilde_PX_meanerr[rap][pt][cpm];
							meandeviation[1][2][4]=meandeviation[1][2][4]+TMath::Abs(pull_lthstar_PX_mean[rap][pt][cpm])/pull_lthstar_PX_meanerr[rap][pt][cpm];
							meandeviation[1][2][5]=meandeviation[1][2][5]+TMath::Abs(pull_lphstar_PX_mean[rap][pt][cpm])/pull_lphstar_PX_meanerr[rap][pt][cpm];


							meandeviation[2][0][0]=meandeviation[2][0][0]+TMath::Abs(pull_lth_CS_sigma[rap][pt][cpm]-1)/pull_lth_CS_sigmaerr[rap][pt][cpm];
							meandeviation[2][0][1]=meandeviation[2][0][1]+TMath::Abs(pull_lph_CS_sigma[rap][pt][cpm]-1)/pull_lph_CS_sigmaerr[rap][pt][cpm];
							meandeviation[2][0][2]=meandeviation[2][0][2]+TMath::Abs(pull_ltp_CS_sigma[rap][pt][cpm]-1)/pull_ltp_CS_sigmaerr[rap][pt][cpm];
							meandeviation[2][0][3]=meandeviation[2][0][3]+TMath::Abs(pull_ltilde_CS_sigma[rap][pt][cpm]-1)/pull_ltilde_CS_sigmaerr[rap][pt][cpm];
							meandeviation[2][0][4]=meandeviation[2][0][4]+TMath::Abs(pull_lthstar_CS_sigma[rap][pt][cpm]-1)/pull_lthstar_CS_sigmaerr[rap][pt][cpm];
							meandeviation[2][0][5]=meandeviation[2][0][5]+TMath::Abs(pull_lphstar_CS_sigma[rap][pt][cpm]-1)/pull_lphstar_CS_sigmaerr[rap][pt][cpm];

							meandeviation[2][1][0]=meandeviation[2][1][0]+TMath::Abs(pull_lth_HX_sigma[rap][pt][cpm]-1)/pull_lth_HX_sigmaerr[rap][pt][cpm];
							meandeviation[2][1][1]=meandeviation[2][1][1]+TMath::Abs(pull_lph_HX_sigma[rap][pt][cpm]-1)/pull_lph_HX_sigmaerr[rap][pt][cpm];
							meandeviation[2][1][2]=meandeviation[2][1][2]+TMath::Abs(pull_ltp_HX_sigma[rap][pt][cpm]-1)/pull_ltp_HX_sigmaerr[rap][pt][cpm];
							meandeviation[2][1][3]=meandeviation[2][1][3]+TMath::Abs(pull_ltilde_HX_sigma[rap][pt][cpm]-1)/pull_ltilde_HX_sigmaerr[rap][pt][cpm];
							meandeviation[2][1][4]=meandeviation[2][1][4]+TMath::Abs(pull_lthstar_HX_sigma[rap][pt][cpm]-1)/pull_lthstar_HX_sigmaerr[rap][pt][cpm];
							meandeviation[2][1][5]=meandeviation[2][1][5]+TMath::Abs(pull_lphstar_HX_sigma[rap][pt][cpm]-1)/pull_lphstar_HX_sigmaerr[rap][pt][cpm];

							meandeviation[2][2][0]=meandeviation[2][2][0]+TMath::Abs(pull_lth_PX_sigma[rap][pt][cpm]-1)/pull_lth_PX_sigmaerr[rap][pt][cpm];
							meandeviation[2][2][1]=meandeviation[2][2][1]+TMath::Abs(pull_lph_PX_sigma[rap][pt][cpm]-1)/pull_lph_PX_sigmaerr[rap][pt][cpm];
							meandeviation[2][2][2]=meandeviation[2][2][2]+TMath::Abs(pull_ltp_PX_sigma[rap][pt][cpm]-1)/pull_ltp_PX_sigmaerr[rap][pt][cpm];
							meandeviation[2][2][3]=meandeviation[2][2][3]+TMath::Abs(pull_ltilde_PX_sigma[rap][pt][cpm]-1)/pull_ltilde_PX_sigmaerr[rap][pt][cpm];
							meandeviation[2][2][4]=meandeviation[2][2][4]+TMath::Abs(pull_lthstar_PX_sigma[rap][pt][cpm]-1)/pull_lthstar_PX_sigmaerr[rap][pt][cpm];
							meandeviation[2][2][5]=meandeviation[2][2][5]+TMath::Abs(pull_lphstar_PX_sigma[rap][pt][cpm]-1)/pull_lphstar_PX_sigmaerr[rap][pt][cpm];
...
//							cout<<meandeviation[0][0][5]<<endl;
	    ...
	    for(int iTab=1; iTab<nTables+1;iTab++){
			...
			if(iTab==1){
				...
				fprintf(NumFile, "$N_{ch}$ & $\\delta_{\\lambda_{\\vartheta}}$ & $\\delta_{\\lambda_{\\varphi}}$ &  $\\delta_{\\lambda_{\\vartheta \\varphi}}$ & $\\delta_{\\tilde{\\lambda}}$ & $\\delta_{\\lambda^*_{\\vartheta}}$ & $\\delta_{\\lambda^*_{\\varphi}}$ \\\\\n");
			}
			if(iTab==2){
				fprintf(NumFile, "\\begin{table}[!h]\n\\centering \\caption{Mean of the distribution of the standard score $z(\\lambda_{i})$: $\\mu_{z(\\lambda_{i})} \\pm \\sigma_{\\mu_{z(\\lambda_{i})}}$}\n\\begin{tabular}{|c|cccccc|}\n\\hline\n");
				fprintf(NumFile, "$N_{ch}$ & $\\mu_{z(\\lambda_{\\vartheta})}$ & $\\mu_{z(\\lambda_{\\varphi})}$ &  $\\mu_{z(\\lambda_{\\vartheta \\varphi})}$ & $\\mu_{z(\\tilde{\\lambda})}$ & $\\mu_{z(\\lambda^*_{\\vartheta})}$ & $\\mu_{z(\\lambda^*_{\\varphi})}$ \\\\\n");
			}
			if(iTab==3){
				...
				fprintf(NumFile, "$N_{ch}$ & $\\sigma_{z(\\lambda_{\\vartheta})}$ & $\\sigma_{z(\\lambda_{\\varphi})}$ &  $\\sigma_{z(\\lambda_{\\vartheta \\varphi})}$ & $\\sigma_{z(\\tilde{\\lambda})}$ & $\\sigma_{z(\\lambda^*_{\\vartheta})}$ & $\\sigma_{z(\\lambda^*_{\\varphi})}$ \\\\\n");
			}
	    ...
	    for(int rapBin = rapBinMin; rapBin < rapBinMax+1; rapBin++) {
	    	...
	    	if(iFrame==3) {sprintf(framerap,"\\hline \\multicolumn{7}{|c|}{PX frame, $%1.1f < |y| < %1.1f$}\\\\ \\hline \\rule{0pt}{4mm}\n",onia::rapForPTRange[rapBin-1],onia::rapForPTRange[rapBin]); fprintf(NumFile,framerap);}
						...
						else fprintf(NumFile, "%1.0f--%1.0f   &  $%1.3f \\pm %1.3f$  & $%1.3f \\pm %1.3f$  &  $%1.3f \\pm %1.3f$ &  $%1.3f \\pm %1.3f$  & $%1.3f \\pm %1.3f$  &  $%1.3f \\pm %1.3f$ \\\\\n",onia::cpmRange[cpmBin-1],onia::cpmRange[cpmBin],lth_tab,ltherr_tab   ,lph_tab,lpherr_tab   ,ltp_tab,ltperr_tab   ,ltilde_tab,ltildeerr_tab   ,lthstar_tab,lthstarerr_tab   ,lphstar_tab,lphstarerr_tab);
		...
		fprintf(NumFile, "\n");
		    ...
		    for(int iTab=1; iTab<nTables+1;iTab++){
				...
				if(iTab==1){
					...
					fprintf(NumFile, "$N_{ch}$ & $\\lambda_{\\vartheta}$ & $\\lambda_{\\varphi}$ &  $\\lambda_{\\vartheta \\varphi}$ & $\\tilde{\\lambda}$ & $\\lambda^*_{\\vartheta}$ & $\\lambda^*_{\\varphi}$ \\\\\n");
				}
		    ...
		    for(int rapBin = rapBinMin; rapBin < rapBinMax+1; rapBin++) {
		    	...
		    	if(iFrame==1) {sprintf(framerap,"\\hline \\multicolumn{7}{|c|}{CS frame, $%1.1f < p_{T} < %1.1f$}\\\\ \\hline \\rule{0pt}{4mm}\n",onia::pTRange[rapBin][ptBin-1],onia::pTRange[rapBin][ptBin]); fprintf(NumFile,framerap);}
		    	if(iFrame==2) {sprintf(framerap,"\\hline \\multicolumn{7}{|c|}{HX frame, $%1.1f < p_{T} < %1.1f$}\\\\ \\hline \\rule{0pt}{4mm}\n",onia::pTRange[rapBin][ptBin-1],onia::pTRange[rapBin][ptBin]); fprintf(NumFile,framerap);}
		    	if(iFrame==3) {sprintf(framerap,"\\hline \\multicolumn{7}{|c|}{PX frame, $%1.1f < p_{T} < %1.1f$}\\\\ \\hline \\rule{0pt}{4mm}\n",onia::pTRange[rapBin][ptBin-1],onia::pTRange[rapBin][ptBin]); fprintf(NumFile,framerap);}
							...
							else fprintf(NumFile, "%1.0f--%1.0f   &  $%1.3f _{-%1.3f}^{+%1.3f} $  & $%1.3f _{-%1.3f}^{+%1.3f}$  &  $%1.3f _{-%1.3f}^{+%1.3f}$ &  $%1.3f _{-%1.3f}^{+%1.3f}$  & $%1.3f _{-%1.3f}^{+%1.3f}$  &  $%1.3f _{-%1.3f}^{+%1.3f}$ \\\\\n",onia::cpmRange[cpmBin-1],onia::cpmRange[cpmBin],lth_tab,ltherr_tab,ltherr_high_tab   ,lph_tab,lpherr_tab,lpherr_high_tab   ,ltp_tab,ltperr_tab,ltperr_high_tab   ,ltilde_tab,ltildeerr_tab,ltildeerr_high_tab   ,lthstar_tab,lthstarerr_tab,lthstarerr_high_tab   ,lphstar_tab,lphstarerr_tab,lphstarerr_high_tab);
