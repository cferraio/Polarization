...
double sigma1S_save[100][100][100];

void GetHisto(Char_t *fileNameIn, Int_t iRapBin, Int_t iPTBin, Int_t iCPMBin);
void FitSignalBG(Double_t nSigma, Int_t iRapBin, Int_t iPTBin, Int_t iCPMBin);
...
void DrawFit(Double_t nSigma, Int_t iRapBin, Int_t iPTBin, Int_t iCPMBin);
void SaveCBParameters(Int_t iRapBin, Int_t iPTBin, Int_t iCPMBin, Double_t alpha, Double_t n, Double_t alphaErr, Double_t nErr);
void SaveFitPars(Int_t iRapBin, Int_t iPTBin, Int_t iCPMBin);
//==============================
void upsilon_2StepFit(Int_t iRapBin = 0,
  ...
  GetHisto(fileNameIn, iRapBin, iPTBin, iCPMBin);
  if(hMass->GetEntries() < 1.){
    printf("\n\n\nskip processing this bin, because the number of entries is smaller than 1\n\n\n");
    return;
  }
  FitSignalBG(nSigma, iRapBin, iPTBin, iCPMBin);
  DrawFit(nSigma, iRapBin, iPTBin, iCPMBin);
}
...
void FitSignalBG(Double_t nSigma, Int_t iRapBin, Int_t iPTBin, Int_t iCPMBin){	
  ...
  sprintf(name, "c1_rap%d_pT%d_cpm%d", iRapBin, iPTBin, iCPMBin);
  ...
  if(iPTBin > 0 && iRapBin > 0 && iCPMBin > 0){
  ...
  sigma1S_save[iRapBin][iPTBin][iCPMBin]=sigma1S;
  ...
  if(iPTBin == 0 && iCPMBin == 0)
    SaveCBParameters(iRapBin, iPTBin, iCPMBin, alpha, n, fRECO->GetParError(6), fRECO->GetParError(5));
...
*/
  if(iRapBin == 0 || iPTBin == 0 || iCPMBin == 0)
    ...
    printf("rapidity bin %d (%1.1f - %1.1f), pT bin %d (%1.0f - %1.0f GeV/c)\n, cpm bin %d (%1.0f - %1.0f GeV/c)\n", 
	   ...
	   iCPMBin, onia::cpmRange[iCPMBin-1], onia::cpmRange[iCPMBin]);
  }
...
//  if(iPTBin > 0 && iRapBin > 0)
	if(iPTBin > -1 && iRapBin > -1 && iCPMBin > -1)
    SaveFitPars(iRapBin, iPTBin, iCPMBin);
